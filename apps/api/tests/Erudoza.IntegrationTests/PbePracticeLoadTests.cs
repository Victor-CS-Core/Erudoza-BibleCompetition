...
public sealed class PbePracticeLoadTests(ITestOutputHelper output)
{
    ...
    public async Task Ten_Pbe_rooms_preserve_six_and_twelve_student_rosters_with_current_authorized_service_commands()
    {
        foreach (var teams in new[] { 1, 2 })
        {
            ...
            output.WriteLine(JsonSerializer.Serialize(report)); var root = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../../../../..")); Directory.CreateDirectory(Path.Combine(root, ".local")); await File.WriteAllTextAsync(Path.Combine(root, ".local", $"c3-canonical-load-{teams}.json"), JsonSerializer.Serialize(report));
        }
    }
}
