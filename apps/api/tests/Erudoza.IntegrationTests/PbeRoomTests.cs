...
public sealed class PbeRoomTests
{
    ...
    public async Task Full_rehearsal_uses_authored_rubrics_active_teams_and_exactly_one_break(int teams, int size, int count, bool recover, bool trustedDraft, bool unarmed = false, bool adultOwner = false, bool invalidReserve = false, bool coached = false, bool cleanupEnd = false, bool simulation = false)
    {
        ...
        for (var q = 0; q < count; q++)
        {
            ...
            if (cleanupEnd && q == 0)
            {
                var revealed = room.GetProperty("results").GetRawText(); if (coached) await Command(-1, "next"); else { time.Advance(TimeSpan.FromSeconds(10)); room = await fixture.Admin.GetFromJsonAsync<JsonElement>(path); }
                Assert.Equal("Presentation", room.GetProperty("phase").GetString()); var abandonedQuestion = room.GetProperty("question").GetProperty("id").GetGuid(); string finals, timings;
                ...
                async Task<string> Snapshot() { using var scope = fixture.Factory.Services.CreateScope(); return (await scope.ServiceProvider.GetRequiredService<ErudozaDbContext>().Set<PracticeRoomRecord>().AsNoTracking().SingleAsync(r => r.Id == roomId)).StateJson; }
                var frozenState = await Snapshot();
                foreach (var action in new[] { "present-ready", "present", "ack", "draft", "submit", "start", "next", "ready" })
                {
                    var caller = coached && action == "present" ? fixture.Admin : fixture.Owner;
                    var rejected = await caller.PostAsJsonAsync(path + "/commands", new { commandId = Guid.NewGuid(), revision = room.GetProperty("revision").GetInt64(), action, questionId = abandonedQuestion, delivery = action == "present" ? coached ? "Coach" : "TextFallback" : null, answers = new[] { "After termination" } });
                    Assert.Equal(HttpStatusCode.BadRequest, rejected.StatusCode); Assert.Equal(frozenState, await Snapshot());
                }
                Assert.NotNull(finalRetry); Assert.Equal(HttpStatusCode.OK, (await finalActor!.PostAsJsonAsync(path + "/commands", finalRetry)).StatusCode); Assert.Equal(frozenState, await Snapshot());
                Assert.Equal(HttpStatusCode.Forbidden, (await fixture.Admin.PostAsJsonAsync(path + "/commands", finalRetry)).StatusCode); Assert.Equal(frozenState, await Snapshot());
        ...
        using (var scope = fixture.Factory.Services.CreateScope())
        {
            ...
            var root = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../../../../..")); Directory.CreateDirectory(Path.Combine(root, ".local")); await File.WriteAllTextAsync(Path.Combine(root, ".local", $"c2-canonical-size-{teams}-{count}-{trustedDraft}.json"), JsonSerializer.Serialize(new { questions = frozen.Questions.Count, reserves = frozen.Reserves.Count, roster = frozen.Members.Count, utf8Bytes = System.Text.Encoding.UTF8.GetByteCount(savedRow.StateJson), status = frozen.Status }));
