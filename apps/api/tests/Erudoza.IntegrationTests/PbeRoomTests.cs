...
public sealed class PbeRoomTests
{
    ...
    public async Task Full_rehearsal_uses_authored_rubrics_active_teams_and_exactly_one_break(int teams, int size, int count, bool recover, bool trustedDraft, bool unarmed = false, bool adultOwner = false, bool invalidReserve = false)
    {
        ...
        var response = await (adultOwner ? fixture.Admin : fixture.Owner).PostAsJsonAsync(fixture.Path + "/rooms", new { seasonId = fixture.SeasonId, teamSize = size, teamCount = teams, questionCount = count, format = "Pbe" }); response.EnsureSuccessStatusCode(); var room = await response.Content.ReadFromJsonAsync<JsonElement>(); var roomId = room.GetProperty("id").GetGuid(); var path = fixture.Path + "/rooms/" + roomId;
        async Task Command(int player, string action, object? payload = null) { var fields = new Dictionary<string, object?> { { "commandId", Guid.NewGuid() }, { "revision", room.GetProperty("revision").GetInt64() }, { "action", action } }; if (payload is not null) foreach (var p in JsonSerializer.SerializeToElement(payload).EnumerateObject()) fields[p.Name] = p.Value; var r = await (adultOwner && action == "invite" ? fixture.Admin : clients[player]).PostAsJsonAsync(path + "/commands", fields); Assert.True(r.IsSuccessStatusCode, await r.Content.ReadAsStringAsync()); room = await r.Content.ReadFromJsonAsync<JsonElement>(); }
        ...
        for (var n = 0; n < ids.Count; n++) await Command(n, "ready"); await Command(0, "start"); var breaks = 0;
        using (var scope = fixture.Factory.Services.CreateScope())
        {
        ...
        for (var q = 0; q < count; q++)
        {
            ...
            for (var team = 0; team < teams; team++) await Command(team * size, "present", new { questionId, delivery = team == 0 ? "Audio" : "TextFallback" });
            ...
            if (q == 0)
            {
                ...
                Assert.Equal(HttpStatusCode.Forbidden, (await fixture.Owner.GetAsync(path)).StatusCode);
            ...
            Assert.Equal("Review", room.GetProperty("phase").GetString()); time.Advance(TimeSpan.FromSeconds(10)); room = await fixture.Owner.GetFromJsonAsync<JsonElement>(path);
            if (room.GetProperty("phase").GetString() == "Break") { breaks++; Assert.Equal(44, q); time.Advance(TimeSpan.FromMinutes(5)); room = await fixture.Owner.GetFromJsonAsync<JsonElement>(path); }
        }
        if (!interrupted)
        {
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<ErudozaDbContext>();
            var savedRow = await db.Set<PracticeRoomRecord>().AsNoTracking().SingleAsync(r => r.Id == roomId); var frozen = PracticeJson.Read<PracticeRoom>(savedRow.StateJson); if (unarmed && !invalidReserve) { Assert.Empty(frozen.Reserves); Assert.Single(frozen.Replacements); } else Assert.Single(frozen.Reserves);
