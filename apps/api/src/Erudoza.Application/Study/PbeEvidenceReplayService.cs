...
public sealed class PbeEvidenceReplayService(IErudozaDbContext db, PbeProgressService progress)
{
    ...
    public Task<object> Continue(PbeDispute d, CancellationToken ct) => progress.ExecuteAsync<object>(async token =>
    {
        ...
        if (next is null) { var correction = await Get(d.OrganizationId, "pbe-dispute-correction", d.Id, token); if (correction is not null) db.PbeTrainingRecords.Remove(correction); var proofs = await Rows(d.OrganizationId, "pbe-target-review").Where(r => ids.Contains(r.Id)).ToListAsync(token); return new { status = "Ready", indexReady = true, d.Revision, proofs = proofs.Select(r => new { revision = r.Revision, proof = Read<PbeReviewProjection>(r) }) }; }
