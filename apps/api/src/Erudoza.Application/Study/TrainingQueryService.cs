...
public sealed class TrainingQueryService(IErudozaDbContext db, TrainingProgressService training, IClock clock)
{
    private async Task<CompetitionSeason?> SeasonAsync(Guid org, Guid student, Guid? id, CancellationToken ct)
    {
        ...
        return await query.Where(x => x.Status == SeasonStatus.Active && db.Assignments.Any(a => a.OrganizationId == org && a.StudentUserId == student && a.SeasonId == x.Id)).OrderBy(x => x.Name).FirstOrDefaultAsync(ct);
    }
