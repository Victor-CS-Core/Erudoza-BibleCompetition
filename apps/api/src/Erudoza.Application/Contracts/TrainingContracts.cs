...
public sealed record SessionResultSummaryDto(Guid AttemptId, int EarnedPoints, int AvailablePoints, DateTimeOffset AcceptedAtUtc, Guid? QuestionId = null, int? OriginalEarnedPoints = null, Erudoza.Application.Study.PbeResultReview? Dispute = null);
public sealed record SessionRecapDto(string Version, Guid SessionId, Guid SeasonId, string Mode, DateTimeOffset? CompletedAtUtc, int Attempted, int Correct, int TargetCardCount, bool FullTargetReached, bool NewlyCreditedDay, string? MissionLocalDate, string? CreditedLocalDate, IReadOnlyList<TrainingStepDto> MissionSteps, IReadOnlyList<BadgeProgressDto> EarnedBadges, IReadOnlyList<PassageChangeDto> PassageChanges, bool Interrupted = false, IReadOnlyList<SessionResultSummaryDto>? Results = null, bool Provisional = false, int PendingCount = 0, int? FinalizedEarnedPoints = null, int? FinalizedAvailablePoints = null);
