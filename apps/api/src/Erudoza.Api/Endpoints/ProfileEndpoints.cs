...
public static class ProfileEndpoints
{
    public static void MapProfileEndpoints(this WebApplication app)
    {
        ...
        group.MapPut("/me/avatar", async (SaveHonorAvatar request, ICurrentUser current, MasteryHonorService profiles, IStudyWriteCoordinator writes, CancellationToken ct) =>
        {
            ...
            catch (CharacterProfileConflictException error) { return Results.Problem(statusCode: StatusCodes.Status409Conflict, title: error.Message); }
        });
        group.MapPut("/me/character", async (SaveCharacterProfile request, ICurrentUser current, MasteryHonorService profiles, IStudyWriteCoordinator writes, CancellationToken ct) =>
        {
            try { return Results.Ok(await writes.ExecuteAsync(current.UserId, token => profiles.SaveCharacterAsync(current.OrganizationId, current.UserId, request, token), ct)); }
            catch (MasteryHonorLockedException error) { return Results.Problem(statusCode: StatusCodes.Status403Forbidden, title: error.Message); }
            catch (CharacterProfileConflictException error) { return Results.Problem(statusCode: StatusCodes.Status409Conflict, title: error.Message); }
        });
