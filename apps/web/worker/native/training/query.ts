...
export async function trainingScope(ctx: RequestContext, requested: string | null) { const assignments = await ctx.store.list<Assignment>('assignment', ctx.orgId, { ownerId: ctx.actor.userId }); const seasons = await ctx.store.list<Season>('season', ctx.orgId); const season = seasons.filter(s => assignments.some(a => a.seasonId === s.id) && (requested ? s.id === requested : s.status === 'Active')).sort((a, b) => (b.createdAtUtc ?? '').localeCompare(a.createdAtUtc ?? ''))[0] ?? null; const sources = season && season.status === 'Active' ? await effectiveSources(ctx, season.id, ctx.actor.userId) : []; return { season, sources, fingerprint: await scopeVersion(sources) }; }
export async function honors(ctx: RequestContext, seasonId: string | null) { const scope = await trainingScope(ctx, seasonId), p = await preference(ctx); const projection = scope.season ? await ctx.store.get<SeasonProgressRecord>('training-season-progress', identity(ctx, scope.season.id, scope.fingerprint), ctx.orgId) : null; const ids = Object.keys((await import('./store')).titles).map(key => identity(ctx, key, 'training-v1', key === 'steady-study' ? 'academy' : scope.season?.id ?? '')); const awards = (await ctx.store.getMany<AwardRecord>('solo-badge-award', ids, ctx.orgId)).map(r => r.value); const counters = projection?.value.counters ?? badgeCounters(scope.sources.map(s => ({ id: kid(s), bookKey: s.bookKey, chapter: s.chapter })), [], [], p?.value.qualifyingWeekStarts.length ?? 0, false); counters['steady-study'] = [Math.min(4, p?.value.qualifyingWeekStarts.length ?? 0), 4]; return catalog(counters, awards); }
export async function today(ctx: RequestContext, seasonId: string | null, now = new Date().toISOString()): Promise<TrainingToday> {
    const old = await preference(ctx), p = resolvePreference(ctx, old, now), c = resolveTrainingCalendar(now, p), scope = await trainingScope(ctx, seasonId);
