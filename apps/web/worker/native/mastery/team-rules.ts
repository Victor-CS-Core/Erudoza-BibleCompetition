import {activeTeams} from '../practice/state';
...
export function teamQualifiers(rooms: Room[], userId: string): { key: HonorKey; evidence: QuestionProof[] }[] {
  const complete = rooms.filter(r => r.format !== 'Pbe' && r.status === 'Completed' && r.completedAt && r.questions.length === r.questionCount && r.submissions.length === r.questions.length * activeTeams(r).length && r.submissions.every(s => s.resolved) && new Set(r.questions.map(q => q.id)).size === r.questionCount && r.questions.every(q => activeTeams(r).every(team => r.submissions.filter(s => s.questionId === q.id && s.team === team).length === 1)) && r.members.some(m => m.userId === userId))
