...
import {validateSimulation,type SimulationSettings} from './simulation';
export const responseDuration=(r:Room,q:Question)=>duration(q)*(r.simulation?.timeMultiplier??1);
...
export function roomScore(r:Room,q:Question,answers:string[],elapsedMs:number,deadline=false){
 ...
 return {accuracyHundredths:rehearsalPoints(gradePbe(q as PbeQuestion,q.parts.map((_,i)=>answers[i]??'')).earnedPoints,elapsedMs/(r.simulation?.timeMultiplier??1),points(q))*100,speedHundredths:0};
}
...
export interface Room {
 simulation?:SimulationSettings;audioReadingComplete?:boolean;
...
export interface Command {simulation?:SimulationSettings;teamSize?:number;questionCount?:number;commandId:string;revision:number;action:string;targetUserId?:string;otherUserId?:string;team?:number;text?:string;answers?:string[];scheduleId?:string;questionId?:string;points?:number;delivery?:'Audio'|'TextFallback'|'Coach'}
...
export function makeRoom(id:string,a:Actor,input:{seasonId:string;teamSize:number;questionCount:number;coached?:boolean;bookKey?:string;format?:'Arcade'|'Pbe';teamCount?:1|2;simulation?:SimulationSettings},epoch:string,now:number):Room {
...
export function recover(r:Room,epoch:string,now:number,reason:string,eligibleReserveIds?:Set<string>):void {
 ...
 if(isPbeRoom(r)&&['Scheduled','Response'].includes(r.phase)){
  for(const team of activeTeams(r))if(!r.submissions.some(s=>s.questionId===current(r).id&&s.team===team)){
   const at=r.draftReceivedAt?.[team];if(at!==undefined&&at>=r.responseStartsAt!&&at<=r.responseStartsAt!+responseDuration(r,current(r))*1000)addSubmission(r,team,r.members.find(m=>m.team===team&&m.scribe)!.userId,r.drafts[team],at-r.responseStartsAt!,true);
   else {r.status='Interrupted';r.phase='Interrupted';r.interruptionReason='ArmedResponsesUntrusted';r.phaseEndsAt=null;return;}
  }
  r.phase=r.simulation?'AnswerLocked':'Review';r.phaseEndsAt=r.simulation?now+3000:r.coached?null:now+10000;return;
 }
 ...
 } else if(r.phase==="Review"&&!r.coached)r.phaseEndsAt=now+10000;else if(r.phase==="Break"&&!r.simulation)r.phaseEndsAt=now+300000;
}
export function advance(r:Room,now:number,pending=false,eligibleReserveIds?:Set<string>):boolean {
 ...
 if(r.phase==="Scheduled"&&now>=r.responseStartsAt!){if(pending)return false;const scribes=r.members.filter(m=>m.scribe);if(scribes.length!==activeTeams(r).length||scribes.some(m=>!r.acknowledged.includes(m.userId))){schedule(r,now);return true;}r.phase="Response";r.phaseEndsAt=r.responseStartsAt!+responseDuration(r,current(r))*1000;return true;}
 if(r.phase==="Response") {
  if(pending)return false;const q=current(r);
  if(now>r.phaseEndsAt!||(now===r.phaseEndsAt!&&!isPbeRoom(r)))for(const team of activeTeams(r))if(!r.submissions.some(s=>s.questionId===q.id&&s.team===team))addSubmission(r,team,r.members.find(m=>m.team===team&&m.scribe)!.userId,r.drafts[team]??[],responseDuration(r,q)*1000,true);
  if(r.submissions.filter(s=>s.questionId===q.id).length===activeTeams(r).length){r.phase=r.simulation?"AnswerLocked":"Review";r.phaseEndsAt=r.simulation?now+3000:r.coached?null:now+10000;return true;}
 }
 if(r.phase==="AnswerLocked"&&now>=r.phaseEndsAt!&&!pending){r.phase="Review";r.phaseEndsAt=now+10000;return true;}
...
export function applyCommand(r:Room,a:Actor,c:Command,ingress:number,now:number,options:{questions?:Question[];invitee?:{id:string;displayName:string};pending?:boolean}={}):void {
 ...
 if(["configure","presenter","move","swap","remove","owner","start","captain","scribe"].includes(c.action))need(c.revision===r.revision,"Room changed. Refresh and retry.",409);
 ...
 switch(c.action){
 case "configure":{lobby();own();need(r.simulation,'Choose a versioned simulation room.');const teamSize=c.teamSize??r.teamSize,count=c.questionCount??r.questionCount;need(Number.isInteger(teamSize)&&teamSize>=2&&teamSize<=6&&teamSize>=r.members.length,'Choose two to six seats, enough for the current roster.');const settings=validateSimulation(c.simulation??r.simulation,count,r.members.map(m=>m.userId));if(teamSize!==r.teamSize||count!==r.questionCount||JSON.stringify(settings)!==JSON.stringify(r.simulation)){r.teamSize=teamSize;r.questionCount=count;r.simulation=settings;resetReady(r);}break;}
 case "presenter":{own();need(r.simulation&&target,'Choose a simulation member.');need(r.status==='Lobby'||r.status==='Playing'&&r.phase==='Presentation','Change presenter in the lobby or before readings.');r.simulation.audioPresenterId=target.userId;r.audioReadingComplete=false;r.presentationDelivery={};r.coachReadyScribeIds=[];if(r.status==='Lobby')resetReady(r);break;}
 ...
 case "present":case "present-ready":{
  ...
  if(r.simulation?.discussion==='InPerson'){
   player();if(c.action==='present'){need(a.userId===r.simulation.audioPresenterId,'Only the designated presenter can confirm readings.',403);need(c.delivery==='Audio'||c.delivery==='TextFallback','Confirm both readings.');r.audioReadingComplete=true;r.presentationDelivery={[a.userId]:c.delivery};if(m!.scribe)r.coachReadyScribeIds=[a.userId];}
   else{need(m!.scribe,'Only the current scribe can confirm readiness.',403);need(!c.delivery,'Readiness does not confirm readings.');r.coachReadyScribeIds=[...new Set([...(r.coachReadyScribeIds??[]),a.userId])];}
   if(r.audioReadingComplete&&scribes.every(id=>r.coachReadyScribeIds?.includes(id))){schedule(r,now);r.acknowledged=scribes;}
  }else if(r.coached){
 ...
 case "submit":{player();need(r.status==="Playing"&&current(r)&&c.questionId===current(r).id,"No matching active question.");need(m!.scribe,"Only the scribe can submit.",403);const original=r.submissions.find(s=>s.questionId===c.questionId&&s.team===m!.team);if(original&&(isPbeRoom(r)||!original.deadlineDraft))break;need(["Scheduled","Response","Review"].includes(r.phase)&&r.acknowledged.length===activeTeams(r).length&&ingress>=r.responseStartsAt!,"Shared response has not started.");validate();const elapsed=ingress-r.responseStartsAt!;
  if(elapsed>responseDuration(r,current(r))*1000){if(!original)addSubmission(r,m!.team,a.userId,r.drafts[m!.team]??[],responseDuration(r,current(r))*1000,true);break;}
  if(original)r.submissions=r.submissions.filter(s=>s!==original);addSubmission(r,m!.team,a.userId,c.answers!,elapsed,false);contribution(true);if(r.phase==="Scheduled"){r.phase="Response";r.phaseEndsAt=r.responseStartsAt!+responseDuration(r,current(r))*1000;}advance(r,now,options.pending??false);break;}
