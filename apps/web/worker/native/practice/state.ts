...
export function applyCommand(r:Room,a:Actor,c:Command,ingress:number,now:number,options:{questions?:Question[];invitee?:{id:string;displayName:string};pending?:boolean}={}):void {
 ...
 case "invite":lobby();need(options.invitee&&options.invitee.id===c.targetUserId&&!r.members.some(x=>x.userId===c.targetUserId),"Player not found or already joined.");need(c.team===undefined||[1,2].includes(c.team),"Choose a team.");need(c.team===undefined||owner||m?.team===c.team,"Invite to your own team.",403);need(!r.invitations.some(i=>i.userId===c.targetUserId&&!i.accepted&&Date.parse(i.expiresAt)>now),"An invitation is already pending.");r.invitations.push({id:crypto.randomUUID(),roomId:r.id,userId:c.targetUserId!,team:c.team,inviterName:a.displayName,expiresAt:new Date(now+86400000).toISOString(),accepted:false});break;
