...
export interface Submission {responseLockedAtMs?:number;questionId:string;team:number;scribeId:string;answers:string[];elapsedMs:number;deadlineDraft:boolean;accuracyHundredths:number;speedHundredths:number;appealed:boolean;resolved:boolean;appealReason?:string}
...
export interface Room {
 ...
 presentations?:Record<string,{scheduleId:string;responseStartsAt:number;responseEndsAt:number;delivery:Record<string,string>}>;
...
export function applyCommand(r:Room,a:Actor,c:Command,ingress:number,now:number,options:{questions?:Question[];invitee?:{id:string;displayName:string};pending?:boolean}={}):void {
 ...
 if(!["submit","ack","draft","present"].includes(c.action))advance(r,now,options.pending);
 ...
 case "present":{player();need(isPbeRoom(r)&&r.phase==='Presentation'&&m!.scribe,'Only the current scribe can confirm the readings.');need(c.questionId===current(r).id&&c.revision===r.revision,'Presentation changed. Refresh and retry.',409);need(c.delivery==='Audio'||c.delivery==='TextFallback','Confirm two readings using audio or text.');
  const state=acknowledgePresentation({questionId:current(r).id,revision:r.revision,delivery:c.delivery,requiredScribeIds:r.members.filter(m=>m.scribe).map(m=>m.userId),readyScribeIds:Object.keys(r.presentationDelivery??{}),responseStartsAtMs:null,responseEndsAtMs:null},a.userId,c.questionId,now,points(current(r)));
  r.presentationDelivery={...r.presentationDelivery,[a.userId]:c.delivery};if(state.responseStartsAtMs!==null){schedule(r,now);r.acknowledged=state.readyScribeIds;}break;}
