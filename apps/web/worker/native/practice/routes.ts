...
import {questionRoutes,setEnabled,enabled} from "./questions";
...
export async function handlePractice(ctx:RequestContext):Promise<Response|null>{
 ...
 if(ctx.path==="/practice/bootstrap"&&ctx.request.method==="GET"){
  ...
  if(!flag?.value.enabled)return json({enabled:false,seasons:[],players:[],rooms:[],invitations:[],achievements:[],questions:[],trends:[]});
  const [seasons,players,rooms,awards,questions,matches]=await Promise.all([ctx.store.list<{id:string;name:string;status:string}>("season",ctx.orgId),ctx.env.DB.prepare("SELECT id,display_name AS displayName FROM Users WHERE org_id=? AND active=1 AND ((kind='Student' AND role='Student') OR (kind='Adult' AND role IN ('Owner','Admin'))) ORDER BY display_name").bind(ctx.orgId).all(),ctx.store.list<Directory>("room",ctx.orgId),ctx.store.list<Award>("award",ctx.orgId,{ownerId:ctx.actor.userId}),admin?ctx.store.list<QuestionRecord>("question",ctx.orgId):Promise.resolve([]),ctx.store.list<Room>("match",ctx.orgId)]);
  return json({enabled:true,seasons:seasons.filter(s=>s.status==="Active"),players:players.results,rooms:rooms.filter(r=>r.ownerId===ctx.actor.userId||r.coachId===ctx.actor.userId||r.memberIds.includes(ctx.actor.userId)||admin&&r.hasAppeals).map(({invitations:_inv,memberIds:_ids,...summary})=>{void _inv;void _ids;return summary;}),invitations:rooms.flatMap(r=>r.invitations.filter(i=>i.userId===ctx.actor.userId&&!i.accepted&&Date.parse(i.expiresAt)>Date.now())),achievements:awards,questions,trends:trends(matches,ctx.actor.userId)});
 }
