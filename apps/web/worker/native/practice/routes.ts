...
export async function handlePractice(ctx:RequestContext):Promise<Response|null>{
 ...
 if(ctx.path==="/practice/bootstrap"&&ctx.request.method==="GET"){
  ...
  const [seasons,players,rooms,questions,matches]=await Promise.all([ctx.store.list<{id:string;name:string;status:string}>("season",ctx.orgId),ctx.env.DB.prepare("SELECT id,display_name AS displayName FROM Users WHERE org_id=? AND active=1 AND kind='Student' ORDER BY display_name").bind(ctx.orgId).all(),ctx.store.list<Directory>("room",ctx.orgId),admin?ctx.store.list<QuestionRecord>("question",ctx.orgId):Promise.resolve([]),listRoomSummaries(ctx.env.DB,ctx.orgId)]);
