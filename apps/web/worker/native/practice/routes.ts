...
export async function handlePractice(ctx:RequestContext):Promise<Response|null>{
 ...
 if(ctx.path==="/practice/bootstrap"&&ctx.request.method==="GET"){
  ...
  const [seasons,players,rooms,questions,matches]=await Promise.all([ctx.store.list<{id:string;name:string;status:string}>("season",ctx.orgId),ctx.env.DB.prepare("SELECT id,display_name AS displayName FROM Users WHERE org_id=? AND active=1 AND ((kind='Student' AND role='Student') OR (kind='Adult' AND role IN ('Owner','Admin'))) ORDER BY display_name").bind(ctx.orgId).all(),ctx.store.list<Directory>("room",ctx.orgId),admin?ctx.store.list<QuestionRecord>("question",ctx.orgId):Promise.resolve([]),listRoomSummaries(ctx.env.DB,ctx.orgId)]);
  ...
  return json({enabled:true,seasons:seasons.filter(s=>s.status==="Active"),players:players.results,rooms:rooms.filter(r=>r.ownerId===ctx.actor.userId||r.coachId===ctx.actor.userId||r.memberIds.includes(ctx.actor.userId)||admin&&r.hasAppeals).map(({invitations:_inv,memberIds:_ids,...summary})=>{void _inv;void _ids;return summary;}),invitations:rooms.flatMap(r=>r.invitations.filter(i=>i.userId===ctx.actor.userId&&!i.accepted&&Date.parse(i.expiresAt)>Date.now()).map(i=>({...i,teamCount:r.teamCount??2}))),achievements:awards,questions:questions.filter(r=>isLegacyQuestion(r.question)),trends:trends(reviewed,ctx.actor.userId)});
 }
