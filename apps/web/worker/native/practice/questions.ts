...
export async function eligibleQuestions(ctx:RequestContext,seasonId:string,count:number,bookKey?:string):Promise<Question[]> {
 ...
 for(const record of records.filter(r=>r.published)){const q=record.question,source=sources.get(q.sourceUnitId);if(!source||source.contentPackId!==q.contentPackId)continue;if((latest.get(q.id)?.version??0)<q.version)latest.set(q.id,q);}
...
export async function questionRoutes(ctx:RequestContext):Promise<Response|null>{
 ...
 if(path==="/questions/import"&&ctx.request.method==="POST"){
  ...
  for(const incoming of input.questions){const q={...incoming,id:incoming.id||crypto.randomUUID()},source=sources.get(q.sourceUnitId);if(!source||source.contentPackId!==q.contentPackId)throw new HttpError(400,"Each question must reference approved season content.");q.reference=source.citation;validateQuestion(q);const key=`${q.id}:${q.version}`;if(keys.has(key))throw new HttpError(409,"This question version already exists.");keys.add(key);const row:QuestionRecord={id:key,seasonId:input.seasonId,published:false,question:q};rows.push(row);}
...
export async function enabled(ctx:RequestContext){const flag=await ctx.store.get<{enabled:boolean}>("practice-setting",ctx.orgId,ctx.orgId);if(!flag?.value.enabled)throw new HttpError(403,"Team Practice is not enabled for this organization.");}
