...
it.each([1,2] as const)('serves full-season material only inside a private %i-team six-seat room and meters composed D1 exposure',async(teams)=>{
 ...
 app=await createNativeTestApp({measureD1:true,delayAuthentication:true,replaceRoomAuthority:true,roomStorageDiagnostics:true,onD1Meter:m=>meters.push(m)});const store=new Store(app.db as unknown as Env['DB']),season=crypto.randomUUID(),pack=crypto.randomUUID();
 ...
 let finalRetry:{actor:number;body:Record<string,unknown>}|undefined;
 const command=async(n:number,action:string,extra:Record<string,unknown>={})=>{const body={commandId:crypto.randomUUID(),revision:room.revision,action,...extra};const r=await call(n,`/rooms/${room.id}/commands`,body);if(action==='submit'&&r.status===200)finalRetry={actor:n,body};expect(r.status,await r.clone().text()).toBe(200);room=await r.json() as PracticeRoom;return r;};
 ...
 if(teams===1){
  const member=players.at(-1)!;const removeMembership=()=>app.db.prepare("DELETE FROM Records WHERE kind='membership' AND id=?").bind(`${season}:${member.userId}`).run();const restoreMembership=()=>store.insert('membership',`${season}:${member.userId}`,TEST_ORG,{seasonId:season,userId:member.userId,difficulty:'Advanced'},{seasonId:season,ownerId:member.userId});
  await removeMembership();const management=await call(-1,`/rooms/${room.id}`);expect(management.status).toBe(200);room=await management.json() as PracticeRoom;expect(room).toMatchObject({materialUnavailable:true,question:null,results:[],draft:[],messages:[]});
  expect((await call(0,`/rooms/${room.id}/commands`,{commandId:crypto.randomUUID(),revision:room.revision,action:'start'})).status).toBe(403);
  await command(-1,'remove',{targetUserId:member.userId});expect(room.members).toHaveLength(5);await restoreMembership();await command(-1,'invite',{targetUserId:member.userId,team:1});
  let inbox=await (await call(5,'/bootstrap')).json() as PracticeBootstrap;room=await (await call(5,`/invitations/${inbox.invitations.find(i=>i.roomId===room.id)!.id}/accept`,{team:1})).json() as PracticeRoom;
  await removeMembership();const left=await call(5,`/rooms/${room.id}/commands`,{commandId:crypto.randomUUID(),revision:room.revision,action:'leave'});expect(left.status).toBe(200);expect(await left.json()).toEqual({left:true});room=await (await call(-1,`/rooms/${room.id}`)).json() as PracticeRoom;
  await restoreMembership();await command(-1,'invite',{targetUserId:member.userId,team:1});inbox=await (await call(5,'/bootstrap')).json() as PracticeBootstrap;room=await (await call(5,`/invitations/${inbox.invitations.find(i=>i.roomId===room.id)!.id}/accept`,{team:1})).json() as PracticeRoom;
 }
 ...
 if(teams===2){
  ...
  const publicationStarted=performance.now(),publicationProgress:unknown[]=[];let stored=await readRoomHistory(app.db as unknown as Env['DB'],TEST_ORG,room.id);for(let retry=0;retry<200&&!stored;retry++){if(retry%10===0){const diagnostic=await app.fetch(`/api/v1/organizations/${TEST_ORG}/practice/rooms/${room.id}`,{headers:{Cookie:players[0].cookie,Origin:'https://erudoza.test','x-test-room-storage':'1'}});const diagnosticData=await diagnostic.json();publicationProgress.push({elapsedMs:performance.now()-publicationStarted,staged:await app.db.prepare('SELECT count(*) AS nodes FROM PracticeRoomComponents WHERE org_id=? AND room_id=?').bind(TEST_ORG,room.id).first(),diagnostic:diagnosticData});await writeFile(new URL('../../../../../.local/c3-history-real-alarm-progress.json',import.meta.url),JSON.stringify(publicationProgress,null,2));}await new Promise(resolve=>setTimeout(resolve,100));stored=await readRoomHistory(app.db as unknown as Env['DB'],TEST_ORG,room.id);}expect(stored?.value.reserves).toHaveLength(1);expect(stored?.value.completedAt).toBeUndefined();expect(stored?.value.submissions.at(-1)?.responseLockedAtMs).toBeTypeOf('number');await writeFile(new URL('../../../../../.local/c2-native-room-size.json',import.meta.url),JSON.stringify({questions:stored!.value.questions.length,reserves:stored!.value.reserves.length,roster:stored!.value.members.length,utf8Bytes:new TextEncoder().encode(JSON.stringify(stored!.value)).length,kind:'match projection after two presented questions; authority also retains messages/drafts/applied commands'}));
 }
 if(teams===1){await new Promise(r=>setTimeout(r,10100));room=await (await call(0,`/rooms/${room.id}`)).json() as PracticeRoom;expect(room.phase).toBe('Presentation');}
 await app.db.prepare("DELETE FROM Records WHERE kind='membership' AND id=?").bind(`${season}:${players[1].userId}`).run();response=await call(0,`/rooms/${room.id}`);expect(response.status).toBe(200);if(teams===1)expect(await response.json()).toMatchObject({materialUnavailable:true,question:null,results:[],draft:[],messages:[]});
 ...
 await app.db.prepare("UPDATE Records SET data=json_set(data,'$.canonicalText','Revoked source revision'),revision=revision+1 WHERE kind='source' AND id=?").bind(sources[1].id).run();const blocked=await call(0,`/rooms/${room.id}`);expect(blocked.status).toBe(200);if(teams===1){expect(await blocked.json()).toMatchObject({materialUnavailable:true,question:null,results:[],draft:[],messages:[]});const preserved=room.results;expect((await call(0,`/rooms/${room.id}/commands`,{commandId:crypto.randomUUID(),revision:room.revision,action:'abandon'})).status).toBe(403);await command(-1,'abandon');expect(room).toMatchObject({status:'Abandoned',question:null,results:[],draft:[],messages:[]});const snapshot=()=>app.fetch(`/api/v1/organizations/${TEST_ORG}/practice/rooms/${room.id}`,{headers:{Cookie:coachCookie,Origin:'https://erudoza.test','x-test-room-snapshot':'1'}}).then(r=>r.text());const frozen=await snapshot();const questionId=JSON.parse(frozen).questions[JSON.parse(frozen).questionIndex].id;
 for(const action of ['present-ready','present','ack','draft','submit','start','next','ready']){const result=await call(action==='present'&&room.coached?-1:0,`/rooms/${room.id}/commands`,{commandId:crypto.randomUUID(),revision:room.revision,action,questionId,...(action==='present'?{delivery:room.coached?'Coach':'TextFallback'}:{}),answers:['After termination']});expect(result.status,action).toBe(400);expect(await snapshot()).toBe(frozen);}
 expect(finalRetry).toBeDefined();const retry=await call(finalRetry!.actor,`/rooms/${room.id}/commands`,finalRetry!.body);expect(retry.status).toBe(200);expect(await snapshot()).toBe(frozen);const wrongActor=await call(1,`/rooms/${room.id}/commands`,finalRetry!.body);expect(wrongActor.status).toBe(403);expect(await snapshot()).toBe(frozen);await new Promise(r=>setTimeout(r,100));expect(await snapshot()).toBe(frozen);
const history=await (await call(-1,`/rooms/${room.id}`)).json() as PracticeRoom;expect(history.results).toEqual(preserved);}
 await writeFile(new URL(`../../../../../.local/c2-native-meter-${teams}.json`,import.meta.url),JSON.stringify(measurements,null,2));
},45000);

it('authorizes real coached reading and current-scribe readiness in both orders',async()=>{
 const meters:{bindingCalls:number;statements:number}[]=[];const measurements:Record<string,{bindingCalls:number;statements:number}>={};
 app=await createNativeTestApp({measureD1:true,replaceRoomAuthority:true,onD1Meter:m=>meters.push(m)});const store=new Store(app.db as unknown as Env['DB']),season=crypto.randomUUID(),pack=crypto.randomUUID();
 await store.insert('practice-setting',TEST_ORG,TEST_ORG,{enabled:true});
 await store.insert('season',season,TEST_ORG,{id:season,organizationId:TEST_ORG,status:'Active',pbeEnabled:true});
 await store.insert('pack',pack,TEST_ORG,{id:pack,isActive:true,licensingStatus:'approved',sourceType:'Scripture'});
 const sources=[];for(let n=1;n<=2;n++){const s={id:crypto.randomUUID(),contentPackId:pack,bookKey:'GEN',chapter:1,verse:n,ordinal:n,citation:`Genesis 1:${n}`,canonicalText:'Alpha',isActive:true};sources.push(s);await store.insert('source',s.id,TEST_ORG,s,{ownerId:pack});}
 const range={bookKey:'GEN',startChapter:1,startVerse:1,endChapter:1,endVerse:2};await store.insert('scope',season,TEST_ORG,{contentPackId:pack,includes:[range],excludes:[]});
 const target=crypto.randomUUID();await store.insert('pbe-target',target,TEST_ORG,{id:target,sourceUnitIds:[sources[1].id],skill:'FactualRecall',label:'Who?'},{seasonId:season,ownerId:sources[1].id});
 for(let n=0;n<12;n++){const q:PbeQuestion={schemaVersion:2,id:crypto.randomUUID(),version:1,contentPackId:pack,sourceUnitId:sources[1].id,sourceUnitIds:[sources[1].id],sourceKind:'Scripture',reference:sources[1].citation,evidence:'Alpha',kind:'ShortAnswer',prompt:'Who?',ordered:false,parts:[{targetId:target,acceptedAnswers:['Alpha'],points:1}]};await store.insert('pbe-question-head',q.id,TEST_ORG,{id:q.id,seasonId:season,published:true,sourceFingerprint:await sourceProof(q,new Map(sources.map(s=>[s.id,s]))),question:q},{seasonId:season,ownerId:sources[1].id});}
 const players=[];for(let n=0;n<4;n++){const userId=crypto.randomUUID(),name=`room-student-${n}`;await app.db.prepare("INSERT INTO Users(id,org_id,user_name,display_name,kind,role,password_hash,credential_version) SELECT ?,org_id,?,?,'Student','Student',password_hash,credential_version FROM Users WHERE id=?").bind(userId,name,name,TEST_USER).run();await store.insert('membership',`${season}:${userId}`,TEST_ORG,{seasonId:season,userId,difficulty:'Advanced'},{seasonId:season,ownerId:userId});await store.insert('assignment',crypto.randomUUID(),TEST_ORG,{seasonId:season,studentUserId:userId,contentPackId:pack,...range,endVerse:1},{seasonId:season,ownerId:userId});const login=await app.fetch('/api/v1/auth/login',{method:'POST',headers:{Origin:'https://erudoza.test'},body:JSON.stringify({identifier:name,password:'Testing!123'})});players.push({userId,cookie:login.headers.get('set-cookie')!.split(';')[0]});}

 const coachCookie=(await app.login()).headers.get('set-cookie')!.split(';')[0];
 const call=async(n:number,path:string,data?:unknown)=>app.fetch(`/api/v1/organizations/${TEST_ORG}/practice${path}`,{method:data?'POST':'GET',headers:{Cookie:n===-1?coachCookie:players[n].cookie,Origin:'https://erudoza.test'},...(data?{body:JSON.stringify(data)}:{})});
 const created=await call(-1,'/rooms',{seasonId:season,teamSize:2,teamCount:2,questionCount:10,format:'Pbe',coached:true});expect(created.status).toBe(200);let room=await created.json() as PracticeRoom;
 let finalRetry:{actor:number;body:Record<string,unknown>}|undefined;
 const command=async(n:number,action:string,extra:Record<string,unknown>={})=>{const body={commandId:crypto.randomUUID(),revision:room.revision,action,...extra};const r=await call(n,`/rooms/${room.id}/commands`,body);if(action==='submit'&&r.status===200)finalRetry={actor:n,body};expect(r.status,await r.clone().text()).toBe(200);room=await r.json() as PracticeRoom;};
 const reject=async(n:number,action:string,extra:Record<string,unknown>,status=403)=>expect((await call(n,`/rooms/${room.id}/commands`,{commandId:crypto.randomUUID(),revision:room.revision,action,...extra})).status).toBe(status);
 for(let n=0;n<4;n++){await command(-1,'invite',{targetUserId:players[n].userId,team:Math.floor(n/2)+1});const inbox=await (await call(n,'/bootstrap')).json() as PracticeBootstrap;const accepted=await call(n,`/invitations/${inbox.invitations[0].id}/accept`,{team:Math.floor(n/2)+1});expect(accepted.status).toBe(200);room=await accepted.json() as PracticeRoom;}
 for(let n=0;n<4;n++)await command(n,'ready');await command(-1,'start');let previous='';
 for(let q=0;q<2;q++){
  const questionId=room.question!.id;
  await reject(0,'present',{questionId,delivery:'Coach'});await reject(1,'present-ready',{questionId});await reject(-1,'present',{questionId,delivery:'Coach',revision:-1},409);
  if(previous)await reject(-1,'present',{questionId:previous,delivery:'Coach'},409);
  const measure=(key:string)=>{const total=meters.splice(0).reduce((a,b)=>({bindingCalls:a.bindingCalls+b.bindingCalls,statements:a.statements+b.statements}),{bindingCalls:0,statements:0});expect(total.statements).toBeLessThanOrEqual(50);measurements[key]=total;};
  if(q===0){meters.length=0;await command(-1,'present',{questionId,delivery:'Coach'});measure('coach-readings');}
  meters.length=0;await command(0,'present-ready',{questionId});measure('first-scribe-'+q);expect(room.phase).toBe('Presentation');meters.length=0;await command(2,'present-ready',{questionId});measure('second-scribe-'+q);
  if(q===1){expect(room.phase).toBe('Presentation');await command(-1,'present',{questionId,delivery:'Coach'});}
  expect(room.phase).toBe('Scheduled');expect(room.coachReading).toMatchObject({questionId,coachId:TEST_USER});expect(room.presentationDelivery).toEqual({[players[0].userId]:'Coach',[players[2].userId]:'Coach'});
  await new Promise(r=>setTimeout(r,3100));room=await (await call(0,`/rooms/${room.id}`)).json() as PracticeRoom;expect(room.phase).toBe('Response');
  await reject(-1,'draft',{questionId,answers:['Alpha']});await reject(-1,'submit',{questionId,answers:['Alpha']});
  await command(0,'submit',{questionId,answers:['Alpha']});await command(2,'submit',{questionId,answers:['Alpha']});expect(room.phase).toBe('Review');expect(room.results.filter(r=>r.questionId===questionId).every(r=>r.accuracyHundredths===100)).toBe(true);
  previous=questionId;await command(-1,'next');
 }
 await app.db.prepare("UPDATE Records SET data=json_set(data,'$.canonicalText','Revoked'),revision=revision+1 WHERE kind='source' AND id=?").bind(sources[1].id).run();const preserved=room.results;await command(-1,'abandon');expect(room.status).toBe('Abandoned');
 const snapshot=()=>app.fetch(`/api/v1/organizations/${TEST_ORG}/practice/rooms/${room.id}`,{headers:{Cookie:coachCookie,Origin:'https://erudoza.test','x-test-room-snapshot':'1'}}).then(r=>r.text());const frozen=await snapshot();const questionId=JSON.parse(frozen).questions[JSON.parse(frozen).questionIndex].id;
 for(const action of ['present-ready','present','ack','draft','submit','start','next','ready']){const result=await call(action==='present'&&room.coached?-1:0,`/rooms/${room.id}/commands`,{commandId:crypto.randomUUID(),revision:room.revision,action,questionId,...(action==='present'?{delivery:room.coached?'Coach':'TextFallback'}:{}),answers:['After termination']});expect(result.status,action).toBe(400);expect(await snapshot()).toBe(frozen);}
 expect(finalRetry).toBeDefined();const retry=await call(finalRetry!.actor,`/rooms/${room.id}/commands`,finalRetry!.body);expect(retry.status).toBe(200);expect(await snapshot()).toBe(frozen);const wrongActor=await call(1,`/rooms/${room.id}/commands`,finalRetry!.body);expect(wrongActor.status).toBe(403);expect(await snapshot()).toBe(frozen);await new Promise(r=>setTimeout(r,100));expect(await snapshot()).toBe(frozen);
const history=await (await call(-1,`/rooms/${room.id}`)).json() as PracticeRoom;expect(history.results).toEqual(preserved);
 await app.db.prepare("DELETE FROM Records WHERE kind='season' AND id=?").bind(season).run();const management=await call(-1,`/rooms/${room.id}`);expect(management.status).toBe(200);expect(await management.json()).toMatchObject({status:'Abandoned',question:null,results:preserved});
 await writeFile(new URL('../../../../../.local/c2-fix1-coached-meters.json',import.meta.url),JSON.stringify(measurements,null,2));
},30000);
