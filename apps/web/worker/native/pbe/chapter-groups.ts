...
export function chapterGroups(sources:PbeSource[],inventory:Map<string,string[]>):ChapterGroup[]{
 ...
 for(const source of sources){const key=source.chapter===null?`intro:${source.contentPackId}`:`chapter:${source.contentPackId}:${source.bookKey}:${source.chapter}`;parents.set(key,[...(parents.get(key)??[]),source]);}
 const result:ChapterGroup[]=[];
 for(const [key,units] of [...parents].sort(([,a],[,b])=>a[0].bookKey.localeCompare(b[0].bookKey)||(a[0].chapter??Number.MAX_SAFE_INTEGER)-(b[0].chapter??Number.MAX_SAFE_INTEGER)||a[0].contentPackId.localeCompare(b[0].contentPackId))){
  units.sort((a,b)=>a.ordinal-b.ordinal||a.id.localeCompare(b.id));const first=units[0],intro=first.chapter===null;
