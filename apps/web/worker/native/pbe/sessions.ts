...
export const pbeSessionDto = (s: PbeSession) => ({ id: s.id, format: s.format, seasonId: s.seasonId, mode: s.mode, status: s.status, targetCardCount: s.cards.length, ruleVersion: s.ruleVersion, scoringVersion: s.scoringVersion, selectionVersion: s.selectionVersion });
...
export const pbeSummary = (s: PbeSession) => ({ sessionId: s.id, format: s.format, mode: s.mode, attempted: s.attempts.length, correct: s.attempts.filter(a => a.result.earnedPoints === a.result.availablePoints).length, targetCardCount: s.cards.length, status: s.status, earnedPoints: s.attempts.reduce((n, a) => n + a.result.earnedPoints, 0), availablePoints: s.attempts.reduce((n, a) => n + a.result.availablePoints, 0), recap: { version: 'pbe-daily-v2', sessionId: s.id, seasonId: s.seasonId, mode: s.mode, completedAtUtc: s.completedAtUtc ?? null, attempted: s.attempts.length, correct: s.attempts.filter(a => a.result.earnedPoints === a.result.availablePoints).length, targetCardCount: s.cards.length, fullTargetReached: s.attempts.length === s.cards.length, newlyCreditedDay: s.newlyCreditedDay, missionLocalDate: s.missionLocalDate, creditedLocalDate: s.creditedLocalDate, missionSteps: pbeMissionSteps(s), earnedBadges: [], passageChanges: [] } });
...
export async function startPbeSession(ctx: RequestContext, input: Start) {
    ...
    if (mode === 'Simulation')
        throw new HttpError(400, 'Timed rehearsal is not enabled. Choose PBE practice.');
    ...
    const selected = selectPbeQuestions({ sessionId, count: 8, mode, trueFalseMaxRatio: 0.1, usedQuestionIds: [], usedTargetIds: [], acceptedTargetGroups: [...groups].sort(([a], [b]) => a - b).map(([, ids]) => [...ids]), candidates: bank.questions.map(q => {
    ...
    if (!selected.length)
        return json({ code: bank.questions.length && mode === 'Review' ? 'PBE_NOTHING_DUE' : 'PBE_COVERAGE_UNAVAILABLE', message: bank.questions.length && mode === 'Review' ? 'No targets are due. Choose Practice.' : 'No eligible published questions are available. Ask your coach to prepare this scope, or choose Memory.', format: 'Pbe' }, 409);
    ...
    const effort = await preparePbeStartEffort(ctx, s, input.training?.timeZone);
    if (s.clientStartId)
...
export async function pbeSessionAction(ctx: RequestContext, sessionId: string, action: string | undefined, input?: unknown): Promise<Response | null> {
    ...
    if (action === 'attempts') {
        ...
        if (previous) {
            ...
            return json({ ...previous.result, alreadyProcessed: true });
        }
        ...
        const grade = gradePbe(card.question, submission.answers), evidence = groupRecallEvidence(card.question, card.targets, submission.answers, attemptId, atMs, unaided), progress = await prepareRecallEvidence(ctx, s.seasonId, s.scopeVersion, evidence, card.question.kind);
        ...
        const attempt: PbeAttempt = { id: attemptId, cardId: card.id, clientSubmissionId: submission.clientSubmissionId, answers: [...submission.answers], hintsUsed: submission.hintsUsed, atMs, result };
        s.attempts.push(attempt);
        s.status = 'Active';
        const effort = await preparePbeEffort(ctx, s, result.acceptedAtUtc);
    ...
    if (!action) {
        ...
        return json({ session: pbeSessionDto(s), card: card ? dto(s, card) : null, attempt: attempt ? { ...attempt.result, alreadyProcessed: true } : null, summary: null });
    }
    return null;
}
