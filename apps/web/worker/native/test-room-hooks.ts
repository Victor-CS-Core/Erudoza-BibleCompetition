...
export function roomTestHooks(source:string,options:{clock?:boolean;replace?:boolean;diagnostics?:boolean}):string {
 ...
 if(!source.includes(body))throw new Error('Room clock hook no longer matches complete bounded body ingress.');
  source=source.replace(body,"if(request.headers.has('x-test-room-now')){const next=Number(request.headers.get('x-test-room-now'));if(!Number.isSafeInteger(next)||next<this.testNow()&&this.ctx.storage.sql.exec('SELECT 1 FROM state WHERE id=1').toArray().length)throw new HttpError(400,'Test clock must advance monotonically.');this.ctx.storage.sql.exec('UPDATE fixture_clock SET now=? WHERE id=1',next);}if(request.headers.has('x-test-room-real-clock')){this.ctx.storage.sql.exec('UPDATE fixture_clock SET real_started=? WHERE id=1 AND real_started IS NULL',this.testPhysicalNow());const due=await this.testGetAlarm();if(due!==null)await this.testSetAlarm(due);}const ingress=Date.now();if(request.headers.has('x-test-room-alarm')){checkOrigin(request,this.env);await authenticate(request,this.env,match[1]);const due=await this.testGetAlarm();if(due===null||due>ingress)throw new HttpError(409,'Test alarm is not due.');await this.alarm();return json(await this.load());}const sensitive=");
 ...
 source=source.replace(constructor,constructor+"ctx.storage.sql.exec('CREATE TABLE IF NOT EXISTS fixture_clock(id INTEGER PRIMARY KEY,now INTEGER NOT NULL,alarm INTEGER,real_started INTEGER)');ctx.storage.sql.exec('INSERT OR IGNORE INTO fixture_clock(id,now,alarm) VALUES(1,?,NULL)',Date.now());");
 // Real-clock mode preserves virtual gameplay time through an offset, while delivering actual platform alarms.
 const methods=`
 private testPhysicalNow():number{return Date.now();}
 private testNow():number{const clock=this.ctx.storage.sql.exec<{now:number;real_started:number|null}>('SELECT now,real_started FROM fixture_clock WHERE id=1').one();return clock.now+(clock.real_started===null?0:Date.now()-clock.real_started);}
 private async testSetAlarm(at:number){this.ctx.storage.sql.exec('UPDATE fixture_clock SET alarm=? WHERE id=1',at);if(this.ctx.storage.sql.exec<{real_started:number|null}>('SELECT real_started FROM fixture_clock WHERE id=1').one().real_started!==null)await this.ctx.storage.setAlarm(Date.now()+Math.max(0,at-this.testNow()));}
 ...
 private async testDeleteAlarm(){this.ctx.storage.sql.exec('UPDATE fixture_clock SET alarm=NULL WHERE id=1');await this.ctx.storage.deleteAlarm();}
 `;
