...
it.each([{ reduce: true, fine: true }, { reduce: false, fine: false }])("keeps patch art still for motion preferences %j", ({ reduce, fine }) => {
  vi.stubGlobal("matchMedia", (query: string) => ({ matches: query.includes("reduced-motion") ? reduce : fine, addEventListener: vi.fn(), removeEventListener: vi.fn() }));
...
it("lifts at the center, dips toward the pointer and cancels pending frames on unmount", () => {
  ...
  fireEvent.pointerEnter(surface, { clientX: 50, clientY: 50 });
  act(() => callback?.(100));
  expect(img.style.transform).toMatch(/translate3d\(0, -[\d.]+px, 0\)/);
  expect(Number.parseFloat(img.style.getPropertyValue("--patch-shadow-y"))).toBeGreaterThan(2);
  fireEvent.pointerMove(surface, { clientX: 100, clientY: 0 });
  act(() => callback?.(116));
  expect(img.style.transform).toMatch(/rotateX\([\d.]+deg\) rotateY\([\d.]+deg\)/);
  expect(Number.parseFloat(img.style.getPropertyValue("--patch-shadow-x"))).toBeLessThan(0);
...
it("rests on pointer exit and immediately resets when motion preferences change", () => {
  ...
  expect(img.style.getPropertyValue("--patch-shadow-y")).toBe("");
  ...
  expect(img.style.getPropertyValue("--patch-shadow-x")).toBe("");
});
