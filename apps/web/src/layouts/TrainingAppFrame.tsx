  ...
  return <div className={`training-app command-app ${coach ? "training-coach" : "training-learner"} ${focused ? "training-focused" : ""}`} data-testid={coach ? "coach-app-shell" : "learner-app-shell"}>
    ...
    <header className="command-masthead"><div className="command-masthead-inner">
      ...
      <NavigationMenu key={`account:${route}`} name="Account" label={<><ProfileAvatar userId={me?.userId ?? ""} displayName={me?.displayName ?? ""} size={48} /><span className="command-account-name">{me?.displayName}</span></>}>
        ...
        <Link to="/wiki"><AppIcon name="book" />Wiki / Help</Link>
    ...
    <div className="training-workspace pathfinder-canvas"><PathfinderBackdrop /><main id="training-main" className="training-main" data-testid={coach ? "coach-main" : "learner-main"}><Outlet /></main><AppFooter supportEnabled={coach && me?.kind === "Adult"} /></div>
