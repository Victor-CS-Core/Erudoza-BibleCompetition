...
function Controls() {
  const auth = useAuth();
  return <><span>{auth.me?.userId ?? "anonymous"}</span><span>{auth.error}</span><button onClick={() => void auth.login("b", "password")}>Login B</button><button onClick={() => void auth.logout()}>Logout</button><button onClick={() => void auth.refresh()}>Refresh</button></>;
}
function setup() {
