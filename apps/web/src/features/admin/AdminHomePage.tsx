...
export function AdminHomePage() {
  ...
  return <div>
    <PageHeader title="Season overview" description="Manage your PBE season and see which students need practice." action={<LinkButton data-testid="create-season" to="/admin/seasons/new">Create season</LinkButton>} />
    {seasons.isPending ? <Notice>Loading seasons…</Notice> : seasons.isError ? <Notice tone="danger"><p>Seasons could not load.</p><Button variant="secondary" onClick={() => void seasons.refetch()}>Try again</Button></Notice> : !selected ? <Panel className="training-empty"><h2>Your team’s season</h2><p>No seasons yet. Create a season to define Scripture scope and assign your students.</p></Panel> : <>
      ...
      {coverage.isError ? <Notice tone="danger"><p>Student progress could not load.</p><Button variant="secondary" onClick={() => void coverage.refetch()}>Retry progress</Button></Notice> : coverage.isPending ? <Notice>Loading student progress…</Notice> : <>
        <div className="training-stats"><Panel as="div"><span className="training-icon-tile"><AppIcon name="users" /></span><div><strong>{studentCount}</strong><small>Assigned students</small></div></Panel><Link to="/admin/students" className="ds-panel training-stat-link" aria-label={`${needReview} ${needReview === 1 ? "student needs" : "students need"} review — open the student directory`}><span className="training-icon-tile training-due"><AppIcon name="review" /></span><div><strong>{needReview}</strong><small>Need review</small></div></Link><Panel as="div"><span className="training-icon-tile"><AppIcon name="book" /></span><div><strong>{selected.assignmentCount}</strong><small>Assignments</small></div></Panel></div>
      ...
      <div className="training-coach-actions"><LinkButton to={`/admin/seasons/${selected.id}?step=students`}>Manage assignments <AppIcon name="arrow" /></LinkButton><p>Assign Scripture.<br /><em>Plan team practice.</em></p></div>
    </>}
  </div>;
}
