...
function SavedPlanner({ seasonId }: { seasonId: string }) {
  ...
  const myAssignments = useQuery({ queryKey: ["my-assignments", org, seasonId], queryFn: () => api.myAssignments(org, seasonId) });
  ...
  // A coach's own study plan ("My assignments") counts toward start
  // readiness: the backend activate route has no student requirement, so a
  // self-only season can start once the coach has a plan.
  const selfAssigned = (myAssignments.data?.length ?? 0) > 0;
  const readyCount = assignedCount + (selfAssigned ? 1 : 0);
  ...
  return <div className="season-planner"><LinkButton variant="ghost" size="compact" to="/admin/seasons">← All seasons</LinkButton><PageHeader title={season.data!.name} description="Your season books and assignments, in one place." action={<Badge tone={active ? "success" : "neutral"}>{season.data!.status === "ContentReady" ? "Books ready" : season.data!.status === "AssignmentsReady" ? "Plans ready" : season.data!.status}</Badge>} />
    ...
    {step === 1 ? <><BookDetails key={seasonId} onDirtyChange={setDirty} season={season.data} initialPacks={data.books.map(({ contentPackId, includes, excludes }) => ({ contentPackId, includes, excludes }))} /><TeamPracticePanel org={org} seasonId={seasonId} /></> : <>
      ...
      <footer className="planner-finish" aria-label="Season actions">
        <div className="planner-finish-help" id="season-finish-help">
          ...
          {!active && !closed && !readyCount && <p>Assign at least one student — or give yourself a study plan under “My assignments” — before starting.</p>}
        </div>
        <div className="planner-actions planner-finish-actions">
          {active || closed ? <Button variant="secondary" disabled={busy || dirty} onClick={() => navigate("/admin/seasons")}>All seasons</Button> : <>
            ...
            <Button disabled={busy || dirty || !data.books.length || !readyCount} aria-describedby="season-finish-help" onClick={() => setConfirmStart(true)}>Start season</Button>
          </>}
        </div>
      </footer>
    </>}
    <div className="planner-actions">{active && <Button variant="secondary" disabled={busy || dirty} onClick={() => setLifecycle("close")}>Close season</Button>}{(active || season.data!.status === "Completed") && <Button variant="secondary" disabled={busy || dirty} onClick={() => setLifecycle("archive")}>Archive season</Button>}<Button variant="danger" disabled={busy || dirty} onClick={() => { removeSeason.reset(); setConfirmDelete(true); }}>Delete season</Button></div>
    ...
    {confirmStart && <ConfirmationDialog title={`Start ${season.data!.name}?`} description={`${assignedCount > 0 ? `${assignedCount} ${assignedCount === 1 ? "student has" : "students have"} assignments${selfAssigned ? ", plus your own study plan" : ""}` : "Your own study plan is the only assignment"}. Starting opens training and locks the season books. Plans can still be updated.`} confirmLabel="Start season" pending={activate.isPending} error={activate.error?.message} onCancel={() => setConfirmStart(false)} onConfirm={() => activate.mutate()} />}
