...
import type { PbeAttemptResult, PbeResumedSession, PbeSessionCard, PbeSubmission } from '../../api/pbeTypes';
...
export function PbeStudyPage({ saved, seasonId, seasonName, unavailable }: {
    ...
    const [session, setSession] = useState<Session | null>(saved?.session ?? null), [card, setCard] = useState<PbeSessionCard | null>(saved?.card ?? null), [result, setResult] = useState<PbeAttemptResult | null>(saved?.attempt ?? null);
    ...
    const next = useMutation({ mutationFn: (id: string) => api.nextPbeCard(id), onSuccess: c => { setCard(c); setResult(null); frozen.current = pending(c.sessionId, c.id); setAnswers(frozen.current?.answers ?? c.question.partPoints.map(() => '')); aid.reset(); } });
    const start = useMutation({ mutationFn: () => api.startSession(seasonId, mode as 'Practice' | 'Review', { clientStartId: startId.current, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone }, 'Pbe'), onSuccess: s => { setSession(s); setParams({ sessionId: s.id, seasonId: s.seasonId, mode: s.mode, format: 'Pbe' }, { replace: true }); } });
    ...
    const submit = useMutation({ mutationFn: () => {
            ...
            return api.submitPbeAttempt(session!.id, frozen.current);
        }, onSuccess: r => { setResult(r); sessionStorage.removeItem(`erudoza:pbe-attempt:${session!.id}`); frozen.current = null; } });
    useEffect(() => {
        ...
        if (mode === 'Simulation' || unavailable)
            return;
    ...
    return <div className="er-study-stage space-y-4"><PageHeader title="PBE practice" description={seasonName} action={<Badge>{mode === 'Simulation' ? 'Timed rehearsal' : `Untimed ${mode.toLowerCase()}`}</Badge>}/>
  {unavailable ? <Panel><Notice>{unavailable}</Notice></Panel> : mode === 'Simulation' ? <Panel><Notice>Timed rehearsal is not enabled.</Notice><LinkButton to={practice}>Start PBE practice</LinkButton></Panel> : <>
