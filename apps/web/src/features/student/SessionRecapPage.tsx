...
export function SessionRecapPage() {
  ...
  const hasCelebration = !!featuredMilestone || !!personalBest?.accuracyBeaten || !!personalBest?.correctBeaten || !!weeklyGoalComplete || !!data?.levelUp || questsCompleted.length > 0;
  const shareText = data && data.version !== "legacy-counts" && hasCelebration
    ? `I completed a ${data.mode} practice session on Erudoza: ${data.correct} of ${data.attempted} correct${personalBest && (personalBest.accuracyBeaten || personalBest.correctBeaten) ? " — a new personal best!" : ""}${featuredMilestone ? ` Earned the ${featuredMilestone.title} milestone!` : ""}${questsCompleted.length ? ` Completed quests: ${questsCompleted.map(quest => quest.title).join(", ")} (+${questsCompleted.length * 25} XP).` : ""}`
    : null;
  const share = async () => {
    if (!shareText) return;
    try {
      ...
      <div className="training-recap-layout">
        <Panel className="training-recap-main">
          ...
          {hasCelebration && <div className="training-recap-celebrations" role="status">
            ...
            {data.levelUp && <p className="training-celebration"><AppIcon name="flag" /><span><strong>Rank up!</strong> {data.levelUp.fromName} → {data.levelUp.toName}</span></p>}
            {questsCompleted.map(quest => <p key={quest.key} className="training-celebration"><AppIcon name="check" /><span><strong>Quest complete: {quest.title}!</strong> +25 XP</span></p>)}
          </div>}
