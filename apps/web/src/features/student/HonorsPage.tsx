...
export function HonorsPage({ hideHeader = false }: { hideHeader?: boolean }) {
  ...
  return <div className="training-dashboard training-collection">{!hideHeader && <PageHeader title="Honors" help="See your Scripture recall achievements and their requirements. Erudoza Honors record Scripture recall and Team Practice achievements. Earn a patch to use it on your profile. These are app achievements, not official Pathfinder Honors." action={<LinkButton variant="secondary" to={trainingLink("/student", seasonId)}>Back to training</LinkButton>} />}
    ...
    {profile.isPending ? <LoadingState label="Loading Honors…" /> : profile.isError ? <Notice tone="danger">Honors could not load. <Button variant="secondary" onClick={() => void profile.refetch()}>Try again</Button></Notice> : <div className="honor-patch-grid" aria-label="Mastery Honors">{visible.map(honor => <HonorPatchCard key={honor.key}
      ...
      detail={<><MasteryHonorArtwork honorKey={honor.key} size={192} muted={!honor.earnedAtUtc} /><p>{honor.category}</p><h3>How to unlock</h3><p>{honor.requirement}</p>{honor.earnedAtUtc ? <><p>Earned <time dateTime={honor.earnedAtUtc}>{evidenceDate(honor.earnedAtUtc)}</time>. Your qualifying evidence is saved.</p><LinkButton variant="secondary" to="/student/profile" aria-label={`Use ${honor.title} as profile image`}>{profile.data?.avatarHonorKey === honor.key ? "Worn as profile image" : "Use as profile image"}</LinkButton></> : <p>This patch is locked. Meet every requirement to unlock it for your profile.</p>}</>}
