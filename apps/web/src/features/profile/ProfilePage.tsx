...
import { PreviewModeToggle, type PreviewMode } from './character/PreviewModeToggle';
...
const CharacterFullscreen = lazy(() => import('./character/CharacterFullscreen').then(module => ({ default: module.CharacterFullscreen })));
    ...
    <Suspense key={rendererVersion} fallback={<LoadingState label="Loading your character editor…"/>}>
      <fieldset className="profile-editor-controls" disabled={save.isPending || reloading}>
        {page === 'Share' && unavailable ? <Panel><h2>Update your choices before sharing</h2><p>Choose Remove unavailable choices above to remove restricted Honors or attire. Your other appearance and card changes will stay in your draft.</p><Button variant="secondary" onClick={() => navigate('Character')}>Review character</Button></Panel>
          ...
          onBackground={background => update('background', background)} onEdit={() => navigate('Character')} onBrowseHonors={() => navigate('Honors')} onError={setRenderError}/>
          : <div className="creator-layout">
            <Panel className="character-panel"><div className="character-panel-full"><PreviewModeToggle mode={previewMode} onChange={setPreviewMode}/>{previewMode === 'animated' ? <AnimatedCharacterView config={config} onError={setRenderError} onActivate={openFullscreen}/> : <CharacterPreview config={config} onError={setRenderError} onActivate={openFullscreen}/>}<p className="figure-caption">Sash · {config.slots.filter(Boolean).length} of 3 Honors selected</p><div className="figure-actions"><Button onClick={() => navigate(page === 'Character' ? 'Profile' : 'Character')}>{page === 'Character' ? 'Back to profile' : 'Edit character'}</Button><Button variant="secondary" onClick={() => navigate('Share')}>Share character</Button></div></div><div className="character-panel-mini"><button type="button" className="character-mini-thumb" onClick={openFullscreen} aria-label="View your character fullscreen"><CharacterPortrait appearance={config} onError={setRenderError}/></button><p className="figure-caption">Sash · {config.slots.filter(Boolean).length} of 3 Honors selected</p><div className="figure-actions"><Button size="compact" onClick={() => navigate(page === 'Character' ? 'Profile' : 'Character')}>{page === 'Character' ? 'Profile' : 'Edit'}</Button><Button size="compact" variant="secondary" onClick={() => navigate('Share')}>Share</Button></div></div></Panel>
            {fullscreenOpen && <CharacterFullscreen config={config} mode={previewMode} onModeChange={setPreviewMode} onClose={() => setFullscreenOpen(false)} onError={setRenderError}/>}
