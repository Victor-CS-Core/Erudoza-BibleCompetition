...
export function isHonorArtwork(key: string) {
  ...
  return key === `${category}:${name}` && ((category === "solo" && solo.has(name)) || (category === "team" && team.has(name)) || (category === "simulation" && simulation.has(name)));
}
export function MasteryHonorArtwork({ honorKey, size = 96, muted = false, className = "" }: { honorKey: string; size?: number; muted?: boolean; className?: string }) {
  ...
  return <PatchArtwork {...(category === "solo" ? honorAsset(name as BadgeKey) : { src: `/brand/${category === "simulation" ? "simulation" : "practice"}/${name}-256.webp`, srcSet: `/brand/${category === "simulation" ? "simulation" : "practice"}/${name}-256.webp 256w, /brand/${category === "simulation" ? "simulation" : "practice"}/${name}-512.webp 512w` })} size={size} muted={muted} className={className} />;
}
