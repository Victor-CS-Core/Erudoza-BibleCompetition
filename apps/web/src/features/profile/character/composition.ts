import {type BodyType,type HairColor,appearanceHead,headPlacement,bodySources,bodyFrame,characterTopInset} from './hair';
import {applyAppearance,paintBackground,paintGroundShadow,backgrounds, type Skin, type Eyes, type Background} from './appearance';
export type Attire = 'student' | 'coach';
import {characterAsset,honorImageSrc} from './assets';
...
export function loadImage(src: string) {
  ...
  if (!task) {
    task = new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      image.onerror = () => { images.delete(src); reject(new Error('Unable to load character artwork. Please try again.')); };
      image.src = src;
    });
    images.set(src, task);
...
export async function renderCharacter(canvas: HTMLCanvasElement, config: Configuration, background: string | null = '#fffefa', quality: 'preview' | 'export' = 'preview') {
  ...
  const [body, garment, accessory, head, mask, scene, ...patches] = await Promise.all([
    loadImage(characterAsset(`prepared/${name}${extension}`)), loadImage(characterAsset(`body-layers/${name}${quality==='export'?'.png':'.webp'}`)), loadImage(characterAsset(`prepared/sash${extension}`)), loadImage(characterAsset(`heads/${headName}${quality==='export'?'.png':'.webp'}`)), loadImage(characterAsset(`heads/${headName}-mask.png`)),
    background?loadImage(quality==='export'?backdrop.fullSrc:backdrop.src):Promise.resolve(null),
    ...config.slots.map(key => {const src=key?honorImageSrc(key):undefined; return src ? loadImage(src) : Promise.resolve(null);}),
  ]);
...
export function setSlot(slots: Slots, index: number, value: string | null): Slots {
  ...
  if (value && !honorImageSrc(value)) throw new Error('Unknown Honor');
