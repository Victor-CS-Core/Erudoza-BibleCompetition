...
export function QuestionEditor({ org, data }: { org: string; data: PracticeBootstrap }) {
  const [conversion, setConversion] = useState<PracticeQuestion>();
  ...
  return <Panel><h2>Coach question bank</h2><p>Use approved season content. Every answer must be supported by Scripture or commentary. Published questions are versioned and frozen when a match starts.</p>
    ...
    <label>Question season<Select disabled={!data.seasons.length} value={selectedSeason} onChange={e => { setSeason(e.target.value); setConversion(undefined); setPack(""); setUnit(""); setPreview(null); }}>{!data.seasons.length && <option value="">No active seasons</option>}{data.seasons.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</Select></label>
    <label>Question format<Select value={format} onChange={e=>setFormat(e.target.value)}><option value="legacy">Existing Team Practice</option><option value="pbe">PBE training</option></Select></label>
    {format === "pbe" ? (selectedSeason ? <PbeQuestionEditor key={`${selectedSeason}:${conversion?.id??"new"}:${conversion?.version??0}`} org={org} season={selectedSeason} initialLegacy={conversion}/> : <p>Create an active season first.</p>) : <>
