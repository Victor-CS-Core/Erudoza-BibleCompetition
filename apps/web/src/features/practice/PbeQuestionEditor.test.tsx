...
it('requires a target before preview and shows the actual selected source',async()=>{
 render(<QueryClientProvider client={new QueryClient()}><PbeQuestionEditor org="org" season="season"/></QueryClientProvider>);
...
it('requires deliberate source and target mapping for a legacy conversion without reusing its identity',async()=>{
 ...
 render(<QueryClientProvider client={new QueryClient()}><PbeQuestionEditor org="org" season="season" initialLegacy={legacy}/></QueryClientProvider>);
...
it('previews multiple sources in the selected citation order',async()=>{
 ...
 render(<QueryClientProvider client={new QueryClient()}><PbeQuestionEditor org="org" season="season"/></QueryClientProvider>);await screen.findByRole('option',{name:'GEN 1:2'});fireEvent.change(screen.getByLabelText('PBE source'),{target:{value:'two'}});fireEvent.click(screen.getByText('Additional source passages'));fireEvent.click(screen.getByRole('checkbox',{name:'GEN 1:1'}));expect(screen.getByRole('blockquote')).toHaveTextContent('Second First');
});
