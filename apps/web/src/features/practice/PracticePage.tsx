...
import {PbeFlagAnswer} from "./PbeFlagAnswer";
...
export function PracticePage() {
  ...
  const base = useLocation().pathname.startsWith("/admin") ? "/admin/practice" : "/student/practice";
...
function PracticeRoomPage({ org, roomId, base }: { org: string; roomId: string; base: string }) {
  ...
  const [firstCoachReading, setFirstCoachReading] = useState<string | null>(null);
  ...
  const act = useCallback(async (action: string, payload: Payload = {}) => {
    ...
    try {
      ...
      const snapshot = !(["submit","draft","present","present-ready","ack"].includes(action)) && hubRef.current?.state === HubConnectionState.Connected ? await hubRef.current.invoke<PracticeRoom>("Command", org, roomId, command) : await practiceApi.command(org, roomId, command);
  ...
  if (base.startsWith("/student") && room.isCoach) return <Panel><PageHeader title="You guide this room" description="This room assigns you the non-playing coach role. Open Coach mode to guide it, or choose another room to play." /><LinkButton to={`/admin/practice/${room.id}?seasonId=${encodeURIComponent(room.seasonId)}`}>Open room in Coach mode</LinkButton><LinkButton variant="secondary" to={base}>Back to Team Practice</LinkButton></Panel>;
  const returnToHub = `${base}?seasonId=${encodeURIComponent(room.seasonId)}`;
  ...
  const availablePlayers = players.data?.players.filter(player => player.id !== room.coachId && !room.members.some(member => member.userId === player.id)) ?? [];
  ...
  const outcome = pbe ? "Rehearsal complete." : !room.results.length || visibleScores.some(score => !score) ? "Match complete." : visibleScores[0]!.totalHundredths === visibleScores[1]!.totalHundredths ? "The scores are tied." : `Team ${visibleScores[0]!.totalHundredths > visibleScores[1]!.totalHundredths ? 1 : 2} ${unresolved ? "leads on points" : "wins on points"}.`;
  ...
  const review = (results: PracticeRoom["results"], expanded = false) => <PracticeAnswerReview results={results} room={room} mineTeam={mine?.team} student={base.startsWith("/student")} pending={pending} act={act} expanded={expanded} appealDrafts={appealDrafts} onAppealDraftChange={(key, reason) => setAppealDrafts(current => ({ ...current, [key]: reason }))} />;
  ...
  const discussion = (mine || room.isCoach) && <Panel className="practice-discussion"><div className="practice-section-heading"><h2>{mine ? `Team ${mine.team} discussion` : "Team discussion"}</h2><Badge>{mine ? "Team only" : "Read-only moderation"}</Badge></div><div className="practice-messages" role="log" aria-label={mine ? `Team ${mine.team} messages` : "Room discussion for moderation"} aria-live="polite">{!room.messages.length && <p className="practice-muted">{mine ? "Discuss the question with your team." : "No discussion messages are available."}</p>}{room.messages.map(message => <article key={message.id}><ProfileAvatar userId={message.userId} displayName={message.displayName} size={32} /><div><strong>{message.displayName}{message.userId === me?.userId && " · You"}</strong><p>{message.text}</p></div></article>)}</div>
    {mine && <form className="practice-chat-compose" onSubmit={e => { e.preventDefault(); void act("chat", { text: chat }).then(success => { if (success) setChat(""); }); }}><label className="practice-suggestion">Suggestion<Input placeholder="Suggest an answer or Scripture reference…" maxLength={500} required value={chat} onChange={e => setChat(e.target.value)} /></label><Button type="submit" disabled={pending || !chat.trim()} aria-label="Share with team"><span aria-hidden="true">↑</span></Button></form>}
  ...
  return <div className="practice-page practice-room" data-status={room.status} data-phase={room.phase}>
    ...
    {room.materialUnavailable && <Panel><h2>Room material is unavailable</h2><Notice>This room’s material or player access has changed. You can still leave or close the room.</Notice><Button variant="secondary" disabled={pending || roomQuery.isFetching} onClick={()=>void roomQuery.refetch()}>Check room access</Button>{lobby && <ul>{room.members.map(member=><li key={member.userId}>{member.displayName}{owner && member.userId!==me?.userId && <Button disabled={pending} variant="ghost" onClick={()=>void act('remove',{targetUserId:member.userId})}>Remove {member.displayName}</Button>}</li>)}</ul>}{lobby && mine && !owner && <Button disabled={pending} variant="secondary" onClick={()=>void act('leave')}>Leave room</Button>}</Panel>}
    {lobby && !room.materialUnavailable && <>
    ...
    {interrupted && <><PageHeader title="Rehearsal interrupted" description={room.interruptionReason==='UnarmedReserveUnavailable'?"No saved replacement question was available. Earlier answers are saved. This rehearsal is incomplete; start a new room to continue practicing.":"One or more teams did not have a trusted response when the room clock was lost. Earlier answers and locked finals are preserved. This is a partial result; no reserve was used for this interruption and full completion was not credited."}/><LinkButton to={`${base}#create-room`}>Start a new rehearsal</LinkButton></>}
    ...
    {playing && !room.materialUnavailable && <div className="practice-live-grid"><Panel className="practice-live-question"><div className="practice-question-meta"><span>{room.question ? `${room.question.points} accuracy points · ${room.question.durationSeconds}s response window` : room.phase}</span>{room.phaseEndsAt ? <span className="practice-timer" role="timer" aria-label={`${room.phase}: ${remaining} seconds remaining`}>{Math.floor(remaining / 60)}:{String(remaining % 60).padStart(2, "0")}</span> : <Badge>{room.coached && ["Presentation", "Review"].includes(room.phase) ? "Awaiting coach" : room.phase === "Paused" ? "Paused" : "Awaiting room update"}</Badge>}</div>
      {room.phase === "Break" ? <><h1>Rehearsal break</h1><p>Five-minute rehearsal break. The next question follows when the break is complete.</p></> : room.phase === "Paused" ? <><h1>The match is paused.</h1><Notice>The interrupted question will be replaced before resuming. Your recorded scores are preserved.</Notice></> : room.question ? <><h1 className="practice-question-prompt">{room.question.prompt}</h1><p className="practice-question-reference">{room.question.reference}</p>
        {room.phase === "Review" ? <><div className="practice-section-heading"><h2>Round review</h2><Badge>{currentResults.length} team answers</Badge></div>{review(currentResults, true)}</> : <>
          {room.phase === "Presentation" && !pbe && <p className="practice-note">Read the question together. {room.coached ? "Your coach will open the response window." : "The shared response window follows presentation."}</p>}{pbe&&room.phase==='Presentation'&&room.coached&&<><Notice>Both coach readings and each current scribe’s readiness are required.</Notice>{room.coachId===me?.userId&&room.isCoach&&!room.coachReading&&(firstCoachReading!==room.question.id?<Button disabled={pending} onClick={()=>setFirstCoachReading(room.question!.id)}>Confirm first coach reading</Button>:<Button disabled={pending} onClick={()=>void act('present',{questionId:room.question!.id,delivery:'Coach'})}>Confirm second coach reading</Button>)}{mine?.scribe&&!room.coachReadyScribeIds?.includes(me!.userId)&&<Button disabled={pending} onClick={()=>void act('present-ready',{questionId:room.question!.id})}>Ready for coach presentation</Button>}</>}{pbe&&room.phase==='Presentation'&&!room.coached&&(mine?.scribe&&!room.presentationDelivery?.[me!.userId]?<PbePresentation key={room.question.id} text={`${room.question.reference}. ${room.question.points} points. ${room.question.prompt}`} onReady={async delivery=>{if(!await act('present',{questionId:room.question!.id,delivery}))throw new Error('Presentation was not accepted.');}}/>:<Notice>Waiting for each current scribe to confirm two readings.</Notice>)}{room.phase === "Scheduled" && <Notice>{pbe?'Response starts in three seconds after all scribes confirm their readings.':'Synchronizing the start. Both scribes acknowledge the shared response window.'}</Notice>}{remaining <= 10 && room.phase === "Response" && room.phaseEndsAt && <Notice>Ten seconds or less remaining.</Notice>}
...
function PracticeAnswerReview({ results, room, mineTeam, student, pending, act, expanded, appealDrafts, onAppealDraftChange }: { results: PracticeRoom["results"]; room: PracticeRoom; mineTeam?: number; student: boolean; pending: boolean; act: (action: string, payload?: Payload) => Promise<boolean | undefined>; expanded: boolean; appealDrafts: Record<string, string>; onAppealDraftChange: (key: string, reason: string) => void }) {
  return <>{results.map(result => <details className="ds-disclosure practice-review" key={`${result.questionId}-${result.team}`} open={expanded || (room.status === "Playing" && result.questionId === room.results.at(-1)?.questionId)}><summary><span>Team {result.team} · {result.prompt}</span><Badge>{points(result.accuracyHundredths)}{room.format==='Pbe'?` / ${points(result.availableHundredths??0)} rubric points`:' accuracy points'}</Badge>{result.dispute&&<Badge tone={result.dispute.status==='Pending'?'warning':'success'}>{result.dispute.status==='Pending'?'Provisional score':'Reviewed score'}</Badge>}{result.appealed ? <Badge tone={result.resolved ? "success" : "warning"}>{result.resolved ? "Appeal resolved" : "Appeal pending"}</Badge> : room.coached && <Badge tone={result.resolved ? "success" : "warning"}>{result.resolved ? "Judgment recorded" : "Judgment needed"}</Badge>}</summary><div className="practice-review-content"><p className="practice-question-reference">{result.reference}</p><p><strong>Submitted:</strong> {result.answers.join(" · ") || "No answer"}</p><p><strong>Accepted:</strong> {result.acceptedAnswers.map(part => part.join(" / ")).join(" · ")}</p><p><strong>Source evidence:</strong> {result.evidence}</p><p>Accuracy {points(result.accuracyHundredths)}{room.format!=='Pbe'&&<> · Speed {points(result.speedHundredths)}</>} · {(result.elapsedMs / 1000).toFixed(2)}s server-observed</p><div className="practice-actions">{room.format==='Pbe'&&student&&mineTeam===result.team&&result.attemptId&&<PbeFlagAnswer activity="Team" sessionId={room.id} attemptId={result.attemptId} review={result.dispute}/>} {room.format!=='Pbe'&&mineTeam === result.team && !result.appealed && (room.status === "Completed" || room.phase === "Review") && <AppealForm reason={appealDrafts[`${result.questionId}-${result.team}`] || ""} onReasonChange={reason => onAppealDraftChange(`${result.questionId}-${result.team}`, reason)} pending={pending} onRequest={text => void act("appeal", { questionId: result.questionId, team: result.team, text })} />}{room.format!=='Pbe'&&room.isCoach && (result.appealed || (room.coached && room.phase === "Review")) && <form className="practice-judgment-form" onSubmit={e => { e.preventDefault(); const form = new FormData(e.currentTarget); void act("judge", { questionId: result.questionId, team: result.team, points: Number(form.get("points")), text: String(form.get("reason")) }); }}><label>Accuracy points<Input name="points" type="number" min={0} step={1} defaultValue={result.accuracyHundredths / 100} required /></label><label>Reason<Input name="reason" required maxLength={500} /></label><Button type="submit" disabled={pending}>Record judgment</Button></form>}{room.format==='Pbe'&&room.isCoach&&<LinkButton variant="secondary" to="/admin/practice/reviews">Open PBE answer reviews</LinkButton>}{student && <LinkButton variant="ghost" to={`/student/study?mode=Review&seasonId=${room.seasonId}`}>Review this season individually</LinkButton>}</div></div></details>)}</>;
}
