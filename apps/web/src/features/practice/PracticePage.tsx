...
function PracticeRoomPage({ org, roomId, base }: { org: string; roomId: string; base: string }) {
  ...
  const canGuide = (room.coached && room.isCoach) || (owner && room.phase === "Paused");
  ...
  return <div className="practice-page practice-room" data-status={room.status} data-phase={room.phase}>
    <div className="practice-room-context">{playing ? <Button variant="ghost" onClick={() => setReturnConfirm(true)}>← All rooms</Button> : <LinkButton variant="ghost" to={base}>← All rooms</LinkButton>}<div><span>{room.teamSize}v{room.teamSize} · {room.questionCount} questions</span><Badge>{room.coached ? "Coach-led" : "Independent"}</Badge></div></div>
