...
function PracticeRoomPage({ org, roomId, base }: { org: string; roomId: string; base: string }) {
  ...
  const teams = [1, 2].map(team => ({ team, members: room.members.filter(member => member.team === team) }));
  ...
  const readiness = !full ? `${room.teamSize * 2 - room.members.length} more ${room.teamSize * 2 - room.members.length === 1 ? "player is" : "players are"} needed.` : !allReady ? `${room.members.length - readyCount} ${room.members.length - readyCount === 1 ? "player is" : "players are"} still preparing.` : "Both teams are ready to begin.";
  ...
  const bothJudged = [1, 2].every(team => currentResults.some(result => result.team === team && result.resolved));
  const canGuide = (base.startsWith("/admin") && room.coached && room.isCoach) || (owner && room.phase === "Paused");
  const canAdvance = room.phase === "Presentation" || room.phase === "Paused" || (room.phase === "Review" && bothJudged) || (room.phase === "Break" && Boolean(room.phaseEndsAt) && remaining === 0);
  ...
  const visibleScores = [1, 2].map(team => room.scores.find(score => score.team === team));
  const outcome = !room.results.length || visibleScores.some(score => !score) ? "Match complete." : visibleScores[0]!.totalHundredths === visibleScores[1]!.totalHundredths ? "An evenly matched finish." : `Team ${visibleScores[0]!.totalHundredths > visibleScores[1]!.totalHundredths ? 1 : 2} ${unresolved ? "leads on points" : "wins on points"}.`;
  ...
  const roster = <div className="practice-teams">{teams.map(({ team, members }) => <Panel key={team} className={`practice-team-card practice-team-${team}`}>
  ...
  return <div className="practice-page practice-room" data-status={room.status} data-phase={room.phase}>
    <div className="practice-room-context">{playing ? <Button variant="ghost" onClick={() => setReturnConfirm(true)}>← All rooms</Button> : <LinkButton variant="ghost" to={returnToHub}>← All rooms</LinkButton>}<div><span>{room.teamSize}v{room.teamSize} · {room.questionCount} questions</span><Badge>{room.coached ? "Coach-led" : "Independent"}</Badge></div></div>
    ...
    {(completed || room.status === "Abandoned") && <details className="ds-disclosure practice-roster"><summary>Team rosters · {room.members.length} players</summary>{roster}</details>}
    {!playing && connection}
    ...
    <p className="practice-footnote">Erudoza PBE preparation: accuracy plus up to 25% speed bonus. Head-to-head scores are a training adaptation, not official PBE standings.</p>
  </div>;
}
...
function PracticeScoreboard({ room, result }: { room: PracticeRoom; result: boolean }) {
  ...
  const team = (teamNumber: number) => {
    ...
    return <div className="practice-score-team"><PracticePatch kind={teamNumber === 1 ? "team-a" : "team-b"} size={result ? 126 : 86} /><div><h2>Team {teamNumber}</h2><strong className="practice-score">{score ? points(score.totalHundredths) : "—"}</strong><p className="practice-subscore">{score ? `${points(score.accuracyHundredths)} accuracy · ${points(score.speedHundredths)} speed` : "No recorded score"}</p></div></div>;
  };
  return <Panel aria-label="Scoreboard" className={`practice-scoreboard${result ? " practice-result-scoreboard" : ""}`}>{team(1)}<div className="practice-match-center"><h2>{room.status === "Playing" ? `Question ${Math.min(room.questionIndex + 1, room.questionCount)} of ${room.questionCount}` : `${scoredQuestions} questions scored`}</h2><p>{room.status === "Playing" ? room.phase : room.status === "Completed" ? "Match complete" : "Incomplete match"}</p><p>{room.status === "Playing" ? "Revealed scores" : "Accuracy + speed"}</p><progress aria-label="Questions scored" value={scoredQuestions} max={room.questionCount} /></div>{team(2)}</Panel>;
}
