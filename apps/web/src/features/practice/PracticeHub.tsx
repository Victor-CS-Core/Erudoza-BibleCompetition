...
import { points } from "./practiceUtils";
import "./practice.css";
  ...
  return <div className="practice-page practice-hub">
    ...
    {data && <>
      {!data.enabled ? <Panel>
        ...
        <div className="practice-hub-grid">
          <Panel className="practice-hub-current" aria-label={currentRoom ? "Current room" : invitation ? "Practice invitation" : "Start Team Practice"}>
            <div className="practice-hub-room-label">
              <span>{currentRoom ? `${seasonName(currentRoom.seasonId)} · ${currentRoom.teamSize}v${currentRoom.teamSize} · ${currentRoom.questionCount} questions` : invitation ? "Your next practice" : "Two teams. One shared goal."}</span>
            ...
            <p>{currentRoom ? coach ? "Two teams, one shared goal. Guide their next practice." : "Gather your team. Put your preparation into practice." : invitation ? `${invitation.inviterName} invited you ${invitation.team ? `to Team ${invitation.team}` : "to a room"}.` : "Create a room or accept an invitation to put your preparation into practice."}</p>
            <div className="practice-hub-duel">
              <div className="practice-hub-team"><PracticePatch kind="team-a" size={132} /><strong>Team 1</strong></div>
              <span className="practice-hub-versus">VS</span>
            ...
            <div className="practice-hub-room-footer">
              ...
              <span>{currentRoom ? <>{currentRoom.memberCount} players · {currentRoom.coached ? "Coach-led" : "Independent"}{currentRoom.ownerId === me!.userId && <><br />You’re the room owner.</>}</> : invitation ? "Join from your invitation below." : "1v1 through 5v5"}</span>
            </div>
          </Panel>
          ...
          <Panel id="create-room" className="practice-hub-create" aria-labelledby="practice-create-title">
            ...
            <form aria-label="Create room" onSubmit={event => {
              ...
              <div className="practice-hub-form-grid">
                ...
                <label>Team size<Select value={teamSize} onChange={event => setSize(Number(event.target.value))}>{[1, 2, 3, 4, 5].map(size => <option key={size} value={size}>{size}v{size}</option>)}</Select></label>
              ...
              <Button type="submit" disabled={!!pending || !selectedSeasonId}>{pending === "create" ? "Creating…" : "Create room"}</Button>
            </form>
          </Panel>
        </div>
        ...
        <div className="practice-hub-lower">
          ...
          <aside className="practice-hub-rules" aria-labelledby="practice-rules-title">
            ...
            <ul><li><strong>Accuracy leads.</strong> Correct answers can earn up to 25% extra as a speed bonus.</li><li><strong>One scribe, one final answer.</strong> Discuss together, then your scribe locks the team’s response.</li><li><strong>Prepare as a team.</strong> Both teams must be full and ready before the owner can start.</li></ul>
            <p>Erudoza PBE preparation: accuracy plus up to 25% speed bonus. Head-to-head scores are a training adaptation, not official PBE standings.</p>
          </aside>
        </div>
      </>}
      ...
      <Panel id="achievements" aria-labelledby="practice-honors-title">
        ...
        <details className="ds-disclosure practice-milestone-history"><summary>Practice milestones</summary><p className="practice-hub-secondary">Earlier team awards retain their original history. These milestones do not unlock profile images.</p><div className="practice-awards">{data.achievements.map(award => <div key={award.key + "-" + award.seasonId}><img src={"/brand/practice/" + award.key + ".svg"} alt="" width={72} height={72} /><strong>{award.title}</strong><small>{seasonName(award.seasonId)}</small><Badge>Milestone recorded</Badge></div>)}</div>{!data.achievements.length && <p className="practice-hub-secondary">No earlier team milestones recorded.</p>}</details>
      </Panel>
      <Panel id="practice-progress" aria-labelledby="practice-progress-title">
        ...
        {data.trends?.map(trend => <article className="practice-review" key={`${trend.seasonId}-${trend.teamSize}-${trend.bookKey}-${trend.ruleVersion}`}>
          <h3>{seasonName(trend.seasonId)} · {trend.teamSize}v{trend.teamSize} · {trend.bookKey || "Season scope"}</h3>
