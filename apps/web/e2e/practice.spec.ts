...
async function openPvpSetup(page: Page, seasonId: string) {
  await page.getByRole("button", { name: "Set up PVP", exact: true }).click();
  const setup = page.getByRole("dialog", { name: "PVP setup", exact: true });
  await expect(setup).toBeVisible();
  await setup.getByRole("combobox", { name: "Season", exact: true }).selectOption(seasonId);
  return setup;
}
async function expectSavedFellowshipAvatar(page: Page, userId: string) {
  const avatar = page.getByRole("button", { name: "Account", exact: true }).locator("[data-profile-user]");
  await expect(avatar).toHaveAttribute("data-profile-user", userId);
  await expect(avatar).toHaveAttribute("data-profile-honor", "team:first-fellowship");
  await expect(page.getByRole("button", { name: "Honor", exact: true })).toHaveAttribute("aria-pressed", "true");
  await expect(page.getByRole("combobox", { name: "Honor profile image", exact: true })).toHaveValue("team:first-fellowship");
  expect(await json(page.request, "/api/v1/profile/me")).toMatchObject({ userId, avatarKind: "honor", avatarHonorKey: "team:first-fellowship" });
}
async function saveFellowshipAvatar(page: Page, userId: string) {
  await page.getByRole("navigation", { name: "Profile pages", exact: true }).getByRole("button", { name: "Honors", exact: true }).click();
  await page.getByRole("button", { name: "Use First Fellowship as profile image", exact: true }).click();
  await expect(page.getByRole("button", { name: "Wearing First Fellowship as profile image", exact: true })).toBeDisabled();
  const saved = page.waitForResponse(response => response.request().method() === "PUT" && response.url().endsWith("/api/v1/profile/me/character"));
  await page.getByRole("button", { name: "Save changes", exact: true }).click();
  expect((await saved).ok()).toBeTruthy();
  await expect(page.getByText("Profile changes saved.", { exact: true })).toBeVisible();
  await page.getByRole("navigation", { name: "Profile pages", exact: true }).getByRole("button", { name: "Profile", exact: true }).click();
  await expectSavedFellowshipAvatar(page, userId);
}
...
test("real browsers invite, synchronize and earn server-measured speed points", async ({ page: admin, browser }, info) => {
  ...
  try {
    for (const [page, route, role] of [[admin, "/admin/practice", "coach"], [a.page, "/student/practice", "student"]] as const) {
      ...
      await expect(page.getByRole("button", { name: "Set up PVP", exact: true })).toBeVisible();
      await capturePractice(page, info, role, "hub");
    }
    await a.page.setViewportSize({ width: 1440, height: 1000 });
    const setup = await openPvpSetup(a.page, fixture.seasonId);
    ...
    await expect(a.page.getByRole("button", { name: "Submit team answer" })).toBeEnabled({ timeout: 30000 });
    await expect(b.page.getByRole("button", { name: "Submit team answer" })).toBeEnabled({ timeout: 5000 });
  ...
  try {
    ...
    for (let round = 0; round < 10; round++) {
      ...
      for (const scribe of [players[0], players[5]]) {
        await expect(scribe.page.getByRole("button", { name: "Submit team answer" })).toBeEnabled({ timeout: 35000 });
        await scribe.page.getByLabel("Answer 1", { exact: true }).fill("Daniel");
        await scribe.page.getByRole("button", { name: "Submit team answer" }).click();
    ...
    await saveFellowshipAvatar(players[0].page, players[0].id);
    await players[0].page.reload();
    await expectSavedFellowshipAvatar(players[0].page, players[0].id);
...
test("coach in Student mode joins by invitation, completes a match and wears an earned Honor in both workspaces", async ({ page: coach, browser }, info) => {
  ...
  try {
    ...
    const setup = await openPvpSetup(other.page, fixture.seasonId);
    await setup.getByRole('button', { name: 'Create room', exact: true }).click();
    ...
    for (let round = 0; round < 10; round++) {
      for (const player of [coach, other.page]) {
        await expect(player.getByRole('button', { name: 'Submit team answer' })).toBeEnabled({ timeout: 35000 });
        await player.getByLabel('Answer 1', { exact: true }).fill('Daniel');
        await player.getByRole('button', { name: 'Submit team answer' }).click();
    ...
    await expect(coach.getByRole('button', { name: 'Use initials', exact: true })).toHaveAttribute('aria-pressed', 'true');
    await saveFellowshipAvatar(coach, me.userId);
    await coach.reload();
    await expectSavedFellowshipAvatar(coach, me.userId);
...
test('PBE six-student independent room reads twice, retries a locked final and rotates the served question',async({page:admin,browser},info)=>{
 ...
 try{
  ...
  await admin.goto('/admin/practice');const coachSetup=await openPvpSetup(admin,fixture.seasonId);await coachSetup.getByRole('combobox',{name:'Practice mode',exact:true}).selectOption('Pbe');await capturePractice(admin,info,'coach','pbe-create');
  await expect(coachSetup.getByText('Two active teams. For one team, use Set up simulation.',{exact:true})).toBeVisible();await coachSetup.getByRole('button',{name:'Create room',exact:true}).click();await expect(admin).toHaveURL(/\/admin\/practice\/[a-f0-9-]+$/);const headToHead=await json(admin.request,`${fixture.path}/rooms/${new URL(admin.url()).pathname.split('/').pop()}`);expect(headToHead).toMatchObject({format:'Pbe',teamCount:2,teamSize:6,coached:false});expect(headToHead.members).toHaveLength(0);
  ...
  await owner.goto('/student/practice');const studentSetup=await openPvpSetup(owner,fixture.seasonId);await studentSetup.getByRole('combobox',{name:'Practice mode',exact:true}).selectOption('Pbe');await expect(studentSetup.getByRole('combobox',{name:'Team size',exact:true})).toHaveValue('6');await expect(studentSetup.getByText('Two active teams. For one team, use Set up simulation.',{exact:true})).toBeVisible();await studentSetup.getByRole('button',{name:'Close dialog',exact:true}).click();
  // Retained one-team rooms still need reading, retry and scoring coverage;
  // current PVP creation above correctly produces two teams.
  const coachRoom=await json(admin.request,fixture.path+'/rooms',{seasonId:fixture.seasonId,format:'Pbe',teamCount:1,teamSize:6,questionCount:10,coached:false});expect(coachRoom).toMatchObject({format:'Pbe',teamCount:1,teamSize:6,coached:false});expect(coachRoom.members).toHaveLength(0);
  ...
  await expect(owner.getByRole('button',{name:'Submit team answer'})).toBeEnabled({timeout:10000});await owner.getByRole('textbox',{name:'Answer 1',exact:true}).fill(answer);await owner.getByRole('button',{name:'Save team draft'}).click();await expect(owner.getByRole('button',{name:'Submit team answer'})).toBeEnabled();
  ...
  await owner.getByRole('button',{name:'Submit team answer'}).click();await owner.getByRole('button',{name:'Retry original action'}).click();await expect(owner.getByRole('button',{name:'Retry original action'})).toHaveCount(0);expect(retryId).toBe(originalId);
...
test('PBE coached presentation waits for two readings and both current student scribes',async({page:admin,browser},info)=>{
 ...
 try{
  ...
  await expect(admin.getByRole('button',{name:'Submit team answer'})).toHaveCount(0);
  for(const n of [0,2]){const student=players[n].page;await expect(student.getByRole('button',{name:'Submit team answer'})).toBeEnabled({timeout:10000});await student.getByRole('textbox',{name:'Answer 1',exact:true}).fill(answer);await student.getByRole('button',{name:'Submit team answer'}).click();}
...
test('PBE deferred review lets students finish and replay before a coach corrects the frozen rubric',async({page:admin,browser},info)=>{
 ...
 try{
  ...
  for(const p of players){room=await command(admin,fixture.path,room,'invite',{targetUserId:p.id});await p.page.goto('/student/practice');await expect(p.page.getByRole('button',{name:'Join Team 2',exact:true})).toHaveCount(0);await p.page.getByRole('button',{name:'Join Team 1',exact:true}).click();await expect(p.page).toHaveURL(url=>url.pathname===`/student/practice/${room.id}`&&url.searchParams.get('seasonId')===fixture.seasonId);room=await json(p.page.request,`${fixture.path}/rooms/${room.id}`);}
  ...
  for(let n=0;n<10;n++){
   ...
   await student.getByRole('button',{name:'I’m ready to hear the question'}).press('Enter');if(n!==1){if(n===0){await expect(student.getByText('Audio is unavailable. Read the question on screen instead. This differs from hearing it read aloud at PBE.',{exact:true})).toBeVisible();await capturePractice(student,info,'student','deferred-text-presentation');}await student.getByRole('button',{name:'Finished first reading'}).press('Enter');await student.getByRole('button',{name:'Finished second reading'}).press('Enter');}else{await expect(student.locator('html')).toHaveAttribute('data-pbe-synthetic-readings','2');await student.evaluate(()=>Object.defineProperty(window,'speechSynthesis',{configurable:true,value:{getVoices:()=>[],addEventListener(){},removeEventListener(){},cancel(){},speak(){}}}));}await expect(student.getByRole('button',{name:'Submit team answer'})).toBeEnabled({timeout:10000});await student.getByRole('textbox',{name:'Answer 1',exact:true}).fill(words[0]);await student.getByRole('textbox',{name:'Answer 2',exact:true}).fill(n===0?'wrong':words[1]);await student.getByRole('button',{name:'Submit team answer'}).click();
  ...
  await student.getByRole('link',{name:'Back to Team Practice',exact:true}).click();const replaySetup=await openPvpSetup(student,fixture.seasonId);await replaySetup.getByRole('combobox',{name:'Practice mode',exact:true}).selectOption('Pbe');await replaySetup.getByRole('button',{name:'Create room',exact:true}).click();await expect(student).toHaveURL(url=>/^\/student\/practice\/[a-f0-9-]+$/.test(url.pathname)&&url.searchParams.get('seasonId')===fixture.seasonId);expect(new URL(student.url()).pathname).not.toContain(room.id);
  ...
  await json(admin.request,fixture.path+'/enabled',{enabled:false});await admin.goto('/admin/practice');await admin.getByRole('link',{name:'Open PBE answer reviews',exact:true}).click();await admin.getByLabel(/Part 2 points/).fill('1');await admin.getByRole('textbox',{name:'Reason for the correction',exact:true}).fill('This saved solo rubric supports both parts.');await admin.route('**/api/v1/pbe/disputes/*/resolve',async route=>{const committed=await route.fetch();expect(committed.ok()).toBe(true);await route.abort('failed');},{times:1});await admin.getByRole('button',{name:'Save rubric correction',exact:true}).click();await expect(admin.getByRole('button',{name:'Continue evidence update',exact:true})).toBeVisible();await admin.reload();await expect(admin.getByRole('button',{name:'Continue evidence update',exact:true})).toBeVisible();await capturePractice(admin,info,'coach','solo-correction-recovery');await admin.getByRole('button',{name:'Continue evidence update',exact:true}).press('Enter');await expect(admin.getByText(/Practice evidence is up to date/)).toBeVisible();await student.getByRole('button',{name:'Refresh reviewed scores',exact:true}).click();await expect(student.getByText('Review resolved',{exact:true})).toBeVisible();await expect(student.getByText('2 / 2 points',{exact:true})).toBeVisible();await expect(student.getByRole('link',{name:'Practice again',exact:true})).toBeVisible();await capturePractice(student,info,'student','solo-corrected-recap');
