...
test('coach switches workspace, assigns personal passages, studies and keeps student coverage separate', async ({ page }, info) => {
  ...
  expect(personal[0].studentUserId).toBe(me.userId);
  ...
  const scripture = await json<{ verses: unknown[] }>(page.request, `/api/v1/study/seasons/${season.id}/scripture`);
  ...
  await page.getByRole('dialog').getByRole('button', { name: 'Remove assignment', exact: true }).click();
  await expect(page.getByText('Assignment removed.', { exact: true })).toBeVisible();
