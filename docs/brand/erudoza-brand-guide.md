...
Field Guide Academy is the paper/cardboard training cover. Learner home and coach/admin home both use `FieldGuideCover`. Coach season readiness shows only real `season.name` and `season.status` from the seasons API. Learner Reviews / Rehearsal tracks, student nav, and study-page start all use the same progress rules (`reviewDueCount`, `seasonStatus`). The cover `DUE` stamp means reviews are due. Do not add a second palette, animation library, or invented readiness scores. See `docs/superpowers/specs/2026-09-07-field-guide-academy-learner-cover-design.md` and `docs/superpowers/specs/2026-09-07-field-guide-academy-track-honesty-design.md`.
