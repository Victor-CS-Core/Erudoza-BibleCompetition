...
function Character({config, label, onError}: {config: Configuration; label: string; onError: (message:string)=>void}) {
  ...
  useEffect(()=>{
    ...
    return ()=>{active=false;};
  },[config.style,config.attire,JSON.stringify(config.slots)]);
...
function App(){
  ...
  const config:Configuration={style,attire,slots};
  const name=`${styleNames[style]}, ${attire==='coach'?'Master Guide':'Pathfinder'} attire, sash, ${slots.filter(Boolean).length} of 3 Honors`;
  async function download(){
    ...
    try{
      ...
      const url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`erudoza-${style}-sash-review.png`;link.click();setTimeout(()=>URL.revokeObjectURL(url),60000);
  ...
  return <>
    ...
    <main>
      <PageHeader title="Make your Pathfinder" description="Try the three styles and choose three Honors for your sash. These choices are samples for review."/>
      ...
      <div className="creator-layout">
        ...
        <div className="editor-panels">
          ...
          <Panel><h2>Wear your Honors</h2><p className="help">Choose up to three Honors for your sash. Empty spots stay dotted.</p>
      ...
      <section className="lineup" aria-labelledby="lineup-title"><PageHeader as="div" titleId="lineup-title" title="Compare all three styles" description="Same attire, sash, and Honor selections."/><div className="lineup-grid">{styles.map(key=><Panel key={key}><Character config={{...config,style:key}} label={`${styleNames[key]}, sash`} onError={setError}/><h3>{styleNames[key]}</h3></Panel>)}</div></section>
