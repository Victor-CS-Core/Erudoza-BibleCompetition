...
const sampleProfile:Readonly<Pick<Me,'userName'|'displayName'>>={userName:'alexbrooks',displayName:'Alex Brooks'};
const profileInitials=sampleProfile.displayName.split(/\s+/).map(part=>part[0]).slice(0,2).join('').toUpperCase();
...
function HairThumbnail({config,onError}:{config:Configuration;onError:(message:string)=>void}){
 const key=[config.bodyType,config.style,config.hairColor,config.skin,config.eyes].join(':');
 const [rendered,setRendered]=useState<{key:string;src:string}|null>(null);
 useEffect(()=>{let active=true;const canvas=document.createElement('canvas');
  renderPortrait(canvas,config).then(()=>{if(active)setRendered({key,src:canvas.toDataURL()});}).catch(e=>{if(active)onError(e.message);});
  return()=>{active=false;};
 },[key,onError]);
 // Hide the previous appearance while its replacement renders. An older job
 // cannot overwrite a newer choice after rapid color or body-type changes.
 const src=rendered?.key===key?rendered.src:null;
 return <span className="hair-thumbnail" aria-busy={!src}>{src&&<img src={src} alt=""/>}</span>;
}
function Character({config,onError}: {config:Configuration;onError:(message:string)=>void}){
 const ref=useRef<HTMLCanvasElement>(null),[readyKey,setReadyKey]=useState('');
 const key=JSON.stringify(config),ready=readyKey===key;
 useEffect(()=>{let active=true;const buffer=document.createElement('canvas');
 renderCharacter(buffer,config).then(()=>{if(active&&ref.current){ref.current.width=buffer.width;ref.current.height=buffer.height;ref.current.getContext('2d')!.drawImage(buffer,0,0);setReadyKey(key);}}).catch(e=>{if(active)onError(e.message);});return()=>{active=false;};},[key]);
 const label=`${config.bodyType} body, ${config.hairColor} hair, ${hairStyles[config.bodyType].find(h=>h.key===config.style)?.name}, ${config.attire==='coach'?'Master Guide':'Pathfinder'}, ${config.skin} skin, ${config.eyes} eyes, sash with ${config.slots.filter(Boolean).length} of 3 Honors`;
...
function App(){
 ...
 return <>
  <header className="review-header"><span className="brand">Erudoza</span><div className="search-area"><Button variant="secondary" className="search-trigger" aria-label="Search" onClick={()=>setSearchOpen(!searchOpen)} aria-expanded={searchOpen}>⌕ <span>Search</span></Button>{searchOpen&&<Panel className="search-results"><Input autoFocus aria-label="Search profile pages" placeholder="Search profile pages" value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>{if(e.key==='Escape')setSearchOpen(false);}}/>{pages.filter(p=>p.toLowerCase().includes(search.toLowerCase())).map(p=><Button key={p} variant="ghost" onClick={()=>navigate(p)}>{p}</Button>)}</Panel>}</div><div className="account-preview"><div className="avatar" aria-label={`Profile image preview: ${avatar}`}>{avatar==='initials'?profileInitials:<img src={avatarSrc} alt=""/>}</div><span>{avatar==='honor'?'Honor':avatar==='character'?'Character':'Initials'}</span></div></header>
  <main><div className="profile-navigation"><span className="breadcrumb">Account <span aria-hidden="true">/</span> {page}</span><nav aria-label="Profile pages">{pages.map(p=><Button key={p} variant="ghost" aria-current={page===p?'page':undefined} onClick={()=>navigate(p)}>{p}</Button>)}</nav></div>
   ...
   {page==='Share'?<ShareEditor config={config} profile={sampleProfile} collection={sampleShareCollection} history={shareHistory} setHistory={setShareHistory} options={shareOptions} setOptions={setShareOptions} onBackground={background=>update('background',background)} onEdit={()=>navigate('Character')} onError={setError}/>:<div className="creator-layout">
