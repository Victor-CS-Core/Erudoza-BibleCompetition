...
function Character({config,onError}: {config:Configuration;onError:(message:string)=>void}){
 ...
 useEffect(()=>{let active=true;setReady(false);const buffer=document.createElement('canvas');
 renderCharacter(buffer,config).then(()=>{if(active&&ref.current){ref.current.width=buffer.width;ref.current.height=buffer.height;ref.current.getContext('2d')!.drawImage(buffer,0,0);setReady(true);}}).catch(e=>{if(active)onError(e.message);});return()=>{active=false;};},[JSON.stringify(config)]);
...
function App(){
 ...
 const [config,setConfig]=useState<Configuration>({bodyType:'male',style:'curls',hairColor:'brown',attire:'student',skin:'medium',eyes:'brown',background:'studio',slots:[honors[0].key,honors[1].key,null]});
 ...
 async function download(){setExporting(true);setError('');setMessage('');try{
  ...
  ctx.fillStyle='#102e47';ctx.font='600 42px system-ui';ctx.textAlign='center';ctx.fillText('My Pathfinder',600,85);ctx.drawImage(character,256,0,1024,1536,130,105,940,1410);ctx.font='42px Georgia';ctx.fillText('Erudoza',600,1550);
