...
try{
 ...
 const mobile=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true});await mobile.goto(page.url());await mobile.waitForFunction(()=>document.querySelector('canvas[data-ready=true]'));await mobile.getByRole('navigation').getByRole('button',{name:'Share',exact:true}).tap();await mobile.waitForFunction(()=>document.querySelector('.share-canvas[data-ready=true]'));await mobile.getByRole('button',{name:'Add First fellowship patch',exact:true}).tap();await expect(mobile.getByRole('button',{name:'Move First fellowship patch',exact:true})).toBeAttached();
