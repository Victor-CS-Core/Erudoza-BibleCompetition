...
- A row's `stamp` is only an actually persisted stamp matching current scope/rule; it stays dated even if a later wrong answer/correction makes `currentReadiness:'Incomplete'`. It never establishes present readiness by itself. Stamps view retains older scopes/rules. `matchesCurrentScope` is true or false only after the current authorized scope identity is verified; it is null when that comparison is not yet verifiable, including a pending rebuild after input changes, disabled admission or unavailable assignment. Historical summaries remain readable during that interval. A runtime that can already verify current inputs may return the known comparison before counters finish; a runtime awaiting its bounded generation may return null. Once the current scope is verified, both runtimes must return the same exact comparison. The page-level `scopeVersion` describes current projection metadata and is not itself a promise that every historical comparison is available or unavailable. No additional full-bank read or projection write is required merely to display historical stamps.
