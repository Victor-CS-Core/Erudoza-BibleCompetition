...
**Execution refinement:** Keep v2 bank rows in native `pbe-question`/`pbe-target` kinds and canonical `PbeTrainingRecord`, leaving `PracticeQuestionRecord` and legacy consumers unchanged. Add focused PBE route/service wiring and DI where required by the HTTP integration tests. The explicit namespace is `/practice/pbe/seasons/{seasonId}` with `/bank` (student counts), `/questions/import`, `/questions/{id}/{version}/publish` and `/enabled`. A3 provides paginated, role-restricted coach authoring reads, standalone target declaration and deliberate legacy conversion through republishing. Coach preparation is independent of the legacy Team Practice flag.
...
- [x] Extend the A1 synthetic question with a second source ID and write this negative boundary test in `bank.test.ts`:
...
- [x] Run `npm --workspace apps/web run test -- worker/native/pbe/bank.test.ts worker/native/pbe/pbe.integration.test.ts --maxWorkers=2`; observe the access/coverage failures before changing query behavior.
- [x] Load published v2 questions with explicit pagination; join declared targets and effective source scope; choose the newest published version by question ID. Leave legacy questions usable in legacy matches but do not invent target equivalence or silently certify them as v2. A3 provides the admin republish flow for conversion. Empty banks return coverage data, not fabricated questions. Add the season enable flag with an absent-value default of false.
- [x] Implement projection storage. Native indexes must match organization/season/owner/kind queries and be idempotent. C# `PbeTrainingRecord` properties are `OrganizationId: Guid`, `Kind: string`, `Id: string`, `SeasonId: Guid`, `OwnerId: Guid?`, `DataJson: string`, `Revision: long`; primary key `(OrganizationId, Kind, Id)`, index `(OrganizationId, SeasonId, OwnerId, Kind)`. Register its DbSet and mapping. Generate with `dotnet ef migrations add PbeTrainingRecords --project apps/api/src/Erudoza.Infrastructure --startup-project apps/api/src/Erudoza.Api --output-dir Persistence/Migrations` using the repository's EF tooling. Verify upgrading an existing database with old sessions and Honors intact.
- [x] Test a bank larger than 5,000 entries using pagination, cross-scope edits during selection and a cached bank whose scope revision changed. Bound reads to the relevant season/source set; record read-budget measurements instead of scanning all student history for each card. Run the native integration file and canonical `PbeQuestionBankTests`; commit `feat: add scoped PBE question banks` after migration/legacy tests pass.

## Task A2b: Represent approved book introductions without verse coordinates

**Why added during execution:** The existing content and assignment APIs require positive chapter/verse ranges and restrict new season books to the built-in NKJV library. A `Commentary` label alone cannot provide the explicitly assigned introduction coverage required by the assessments. Use an additive PBE source path; leave Scripture locators and legacy content unchanged.

**Files — create:** native `pbe/sources.ts`, `sources.test.ts`, `introductions.integration.test.ts`; canonical domain `PbeIntroduction.cs`, application `Study/PbeIntroductionService.cs` and `PbeSourceResolver.cs`, integration `PbeIntroductionTests.cs`. Extend A2's public authoring contracts, bank resolver and focused endpoints/DI. These paths are under the projects and directories in the index. Reuse A2 storage; no new SQL columns or fabricated `SourceUnit` rows are needed.

**Contracts:** An immutable introduction pack stores organization/season scope, server-generated pack/source GUIDs, book key, source edition, title, citation, licensing status, reviewed state and ordered source units `{id, citation, canonicalText}`. It has no chapter/verse fields. Store it as `pbe-introduction` in native Records / canonical PbeTrainingRecord. The pack ID is its PBE `contentPackId`; source-unit IDs can be referenced by A1 targets/questions with `sourceKind: Commentary`. A corrected source creates a new pack/source identity, retaining prior evidence. Per-student `pbe-introduction-assignment` records use deterministic season/pack/student identities and explicit owner IDs.

- [x] Write failing native/canonical HTTP tests that a coach can prepare an introduction for a book in the selected season without chapter/verse input, explicitly mark its source reviewed, and assign it to selected active season members. A student outside that selection must not receive its questions or source text.
- [x] Add role-restricted season endpoints for introduction create/list/review and explicit assignment replacement. Accept bounded coach-provided text, edition/citation and license metadata; do not fetch copyrighted material or substitute sample text for the actual source. Require a selected season book and approved licensing/source review before publishing questions. Keep source IDs immutable; never modify built-in NKJV packs.
- [x] Extend the focused PBE source resolver to combine allowed Scripture ranges with reviewed, deliberately assigned introductions. Team callers use season coverage; solo callers use the student's explicit scope. Return a common PBE source view with nullable/absent Scripture coordinates and a clear source kind. Revalidate selected book, active season, membership, source approval and assignment revisions; no cached permission survives an edit. Canonical and native behavior must agree.
- [x] Prove that publishing a Commentary question binds its rubric evidence to the actual introduction source and rejects a Scripture/Commentary type mismatch. Removing a book, revoking approval or changing introduction assignments excludes it from future selection while leaving saved historical question/evidence records unchanged. Cover foreign organizations, unreviewed sources, duplicate assignment retries and stale concurrent writes.
- [x] Run focused native/canonical source/bank tests and both type checks. Record actual source/query and authorization evidence; commit `feat: add explicitly assigned PBE introductions`. A3 supplies its coach UI; B2 consumes the common resolver; D2 renders `intro:<contentPackId>` groups without inventing coordinates. Actual selected-season source review remains a separately coordinated pilot prerequisite.

## Task A3: Prepare banks before play and expose honest coverage

**Files — create:** `apps/web/src/features/practice/PbeTargetFields.tsx`, `PbeTargetFields.test.tsx`, `PbeBankCoverage.tsx`, `PbeBankCoverage.test.tsx`, `PbeIntroductionEditor.tsx`, `PbeIntroductionEditor.test.tsx`.
