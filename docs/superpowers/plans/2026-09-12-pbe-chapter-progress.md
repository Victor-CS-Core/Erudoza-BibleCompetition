...
// projectChapter(scope, targets, reviews, proofs, questions, context: { asOfMs: number }): ChapterProgress
```

`scope` is `{ chapterKey: string; scopeVersion: string; assignedSourceUnitIds: string[]; eligibleAssignmentSourceUnitIds: string[] }`; `targets` are Phase A `PbeTarget[]`, `reviews` are Phase B `TargetReview[]`, `proofs` are `TargetProof[]`, and `questions` are eligible published Phase A `PbeQuestion[]`. `assignedSourceUnitIds` contains this chapter/group's sources and must be a subset of the complete current personally eligible `eligibleAssignmentSourceUnitIds`. A target/question must lie wholly within that eligible assignment, then contributes to each chapter/group it intersects; count covered passages only inside the current group. Do not silently drop a declared cross-chapter target when both sides are assigned, or count a spanning target/question that includes an excluded source. Test both cases. Supply a required sixth context argument `{ asOfMs: number }` from the trusted caller to compute due counts deterministically; never use ambient time or infer current time from proof history. The pure projection returns `stampEarnedAtUtc: null`; the transactional adapter supplies an actual persisted first-earned stamp after consistent qualification. `TargetProof.recall` reflects the original frozen rubric's applicability to that target skill; current question heads serve current coverage only and cannot reinterpret old proof. Count distinct recall-capable question IDs per target from `questions` to derive missing variants; recognition-only TrueFalse questions do not satisfy this prerequisite. Carry the original server accepted sequence from B1 into each proof and replay by acceptance order, including equal UTC millisecond timestamps; correction resolution time is not a new retrieval. Keep first earned stamp evidence immutable; current counters recompute from final evidence and the current scope. For timed solo attempts, `TargetProof.atMs` uses the original trusted response lock time (`responseLockedAtMs` native / `ResponseLockedAtUtc` canonical); otherwise it uses original acceptance time. Keep `acceptedSequence` as the replay ordering key. A delayed outbox projection cannot manufacture the required retrieval delay. Add a regression with a response locked early but projected more than 48 hours later, plus a genuinely delayed later response. Do not rewrite stored acceptance times.

- [x] Write this delayed/fresh-question test:
...
- [ ] Add selectable chapter groups of roughly 3–5 assigned verses, splitting at excluded gaps and book boundaries. Render approved commentary as a separately labeled book-introduction group, with a stable `intro:<contentPackId>` group key and target IDs; do not invent chapter/verse coordinates. Show practiced/recalled/retained counts with question coverage and next due targets. Use B2's scoped action to continue a chapter or B1's repair targets for a small comeback quest. If an old Honor requires more passages, higher difficulty or a legacy activity/rule profile, explain its exact prerequisite while offering achievable chapter progress; never enlarge assignments to unlock it. In particular, do not imply that collaborative PBE team answers qualify for a legacy Honor that claims personal accuracy from scribe submissions; retain the old earned Honor and show the applicable activity context.
...
- [ ] Include the added B4 MissingWords flow: type at adjacent/separated inline blanks, leave a middle field empty, verify positional grading, restore a structured pending request and retry without changed identity/timing. Preserve a legacy string-only pending request unchanged. The populated migration/export fixtures must retain canonical `Attempt.AnswerPayloadJson`, its additive EF migration and the native versioned `answerPayload`, with original indexed text and no regrading of historical attempts.
...
- [ ] Preserve populated PBE export/import contracts, including the actual `PbeTrainingRecords` composite identity, revisions, season enable flags, raw JSON dictionary keys and C3's accepted bounded room format. Keep the 100KB SQL statement guard; use a separately verified bound-parameter import path for larger supported records. An original archive above D1's 2MB row limit needs a reviewed lossless chunk/manifest representation with exact reconstructed bytes and composite source identity, or explicit unsupported conversion with its full source backup retained. Archive counts alone do not prove operational native history, authority recovery or retry. Validate complete bundles before mutation and exact same-bundle resume against an isolated offline target; no live import is implied.
...
- [ ] The user's subsequent all-branch instruction supplies final main integration, deployment and branch-cleanup authorization. After the required local gates, reconcile all local/remote branch progress and uncommitted work, preserve unique history in main, apply the reviewed migration, publish the verified combined build and read back its active version/bindings. Perform separate public and authenticated smoke checks with authorized accounts, proving new PBE start, answer, finish, replay and preserved old results. Record exactly which live steps ran. Real pilot contacts and silent enabling of an unreviewed season bank remain outside that instruction.
- [ ] After verified deployment and exact remote main readback, re-inventory branches/worktrees, confirm every branch tip is reachable from main and no uncommitted work is lost, then remove non-main local/remote branch refs. Preserve private backups/tooling and active previews; a branch ref can be removed after safely detaching a clean worktree without deleting its files.
