...
Return optional `missingWordResults` and `missingWordAnswers` on the accepted attempt result/DTO from the saved server-generated results and original raw payload answers, respectively. Feed saved results into the component's `results` prop and restore submitted field values only after the existing feedback boundary; preserve wrong and empty raw answers on accepted reload without reconstructing them from expected text. The client request contains answers only; it cannot supply expected text or correctness. Persist the original per-slot results so later code changes cannot silently reinterpret historical feedback.
