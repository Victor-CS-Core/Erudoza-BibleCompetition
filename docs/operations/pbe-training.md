...
Include chapter/cooperation refresh cost in that assessment. D1's local 10,000-target fixture completes, but one generation consumes 13,728,367 known rows read, excluding queries whose meter lacks row metadata. This exceeds the current [Free D1 daily allowance](https://developers.cloudflare.com/d1/platform/pricing/) of five million rows read. Ordinary request/page bounds therefore do not establish a safe daily budget. D3 must measure a realistic cohort and refresh frequency, including retained history and writes, before enablement. Preserve disabled defaults during deployment if that capacity gate remains open; no paid upgrade is authorized.
...
The existing SQL exporter retains a 100KB statement guard. Parameter binding is required for larger supported rows, but D1 still has a [2MB row/string/BLOB limit](https://developers.cloudflare.com/d1/platform/limits/). C3's bounded native room format and connection-probe fix are locally verified and source-reviewed, and D3's private local importer is implemented for the documented A–D1 subset; D2-containing and hosted backups remain unsupported.
...
The [local restore implementation contract](pbe-restore-contract.md) defines the initial terminal/drained authority boundary, private local adapter, exact archives and operating caps; its implemented local subset and explicit unsupported boundaries are recorded above. Initial canonical conversion also rejects assignments with multiple scope rows before target writes, preserving the complete original backup; single contiguous scopes may span chapters. It must not split identities and invalidate saved chapter evidence. The accepted importer must prove native authenticated history, resume, retry, pending correction and projection recovery from actual populated fixtures. Archived rows and matching counts alone cannot establish those behaviors. This is one-way conversion; switching back to a legacy host without reconciling newer native attempts is not a safe rollback.

## Authorized rollout and rollback

The user subsequently authorized final all-branch integration, deployment and branch cleanup; see the [consolidation record](2026-09-12-branch-consolidation.md). After the local gate, use the reviewed deployment-specific configuration and private backup. Retain feature defaults off until the chosen cohort is ready. Apply only reviewed additive migrations and the exact verified combined build, then read back the active version and bindings. Run separate public and authorized authenticated smoke checks for start, answer, finish, replay, original-result preservation and privacy. Record which actions actually ran; no real invitations or messages are implied. Remove other branches only after their progress is preserved in main and deployment is verified.
...
## Initial local restore tool boundary

The implemented private local importer has populated canonical Room, Solo correction and chapter recovery evidence, 90 current pure validation checks, and a two-Room interrupted/lost-acknowledgment restore. Ordinary complete native source preflight passes. The large-answer source exceeds the unchanged total payload cap and is rejected with its backup retained. D2-containing backups remain explicitly unsupported; further converter development is outside this release under the user scope freeze. No hosted restore is offered by this local tool.
