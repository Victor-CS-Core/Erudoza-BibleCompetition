...
|---|---|
| Exact reviewed release commit and pushed remote readback | Pending final C3/D review; C2 checkpoint is historical evidence only |
| Ordered native migration names and hashes | `0005_pbe_training.sql` exists; final storage/schema inventory pending |
...
| Canonical EF migration and populated conversion mapping | `20260912021321_PbeTrainingRecords`; final C3/D record mapping pending |
...
| Six/twelve-student full rehearsal and local load measurements | Pending final C3 gate |
...
The existing SQL exporter retains a 100KB statement guard. Parameter binding is required for larger supported rows, but D1 still has a [2MB row/string/BLOB limit](https://developers.cloudflare.com/d1/platform/limits/). C3's bounded native room format and D3's importer are pending; there is no approved command here for importing the new populated PBE release yet.
