...
The implementation returns unavailable when configuration is incomplete. Do not enable public onboarding until a verified sender and an explicitly authorized delivery test work. Resend account access and real delivery validation are still pending. The native security secrets and public site key are deployed; `RESEND_API_KEY` and `AUTH_EMAIL_FROM` remain absent, so account email actions are unavailable. The retained .NET backend has no new onboarding endpoints; its UI treats that capability as unavailable.
...
Remaining activation work: connect a Resend Free account, verify the sending subdomain using additive DNS records, store its sending key, configure the approved sender, and complete an explicitly authorized real email/Turnstile flow. No paid plans were enabled.
