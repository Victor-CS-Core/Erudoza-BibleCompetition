...
The implementation returns unavailable when configuration is incomplete. Production email onboarding is now enabled: the native security secrets, public site key, restricted `RESEND_API_KEY`, and `AUTH_EMAIL_FROM=Erudoza <accounts@auth.erudoza.com>` are deployed. The sender is also in versioned production configuration so future deployments preserve it. Resend sender verification, authorized delivery preflight and the real signup-code/Turnstile flow passed on September 11. The retained .NET backend has no new onboarding endpoints; its UI treats that capability as unavailable.
...
At preparation checkpoint `b5abcb5` (and pushed-log follow-up `d61f3d8`), real delivery authorization was still pending; no email had been sent. That gate is now resolved below. An app 202 or stored `delivered=1` proves provider acceptance only; check provider delivery status and recipient receipt. Do not complete signup, change passwords or send third-party invitations on a user's behalf without authorization.

## Live email activation — September 11

The user authorized the setup email and verification-code test to their own Resend account address. Exactly two messages were sent:

| Message | Provider ID | Result |
|---|---|---|
| Erudoza email setup test | `1b0060e7-98be-488e-9edc-538ba60bc033` | Resend API accepted at 16:47:48 UTC; dashboard then showed delivered. |
| Your Erudoza verification code | `86bbdaa5-347c-45f0-9c68-a32841a001fc` | Sent through the live signup page after real Turnstile success; dashboard showed delivered. |

The preflight used a temporary loopback-only, single-send form forwarding the held key to the official Resend API. It accepted only the fixed authorized recipient/message, enforced same-origin submission and a stable idempotency key, kept the key in memory, logged only sanitized results, and shut down after sending. No fixed code, test provider, inbox preview or bypass was added to production.

The sender configuration passed a Wrangler dry run and deployed as Worker **`1e9d199c-3562-4706-897d-daf92e46e05f`**. No updated frontend assets were uploaded. Real signup advanced to the verification-code form, focused the code input, displayed a ten-minute expiry and disabled its resend button for the cooldown. The agent stopped at that form. The user then completed their own signup in the browser, which reached `/admin`; a bounded indexed read confirmed an active **Adult / Owner** in a club separate from the preserved academy. The stored signup challenge had a ten-minute lifetime and was consumed at 16:52:46 UTC. No password was changed by the agent, and no third-party invitation was sent. Inbox versus spam placement has not been separately confirmed.

Post-activation smoke passed at **16:52:46 UTC**: native API/D1 health, available email capability with the correct site key, exact unchanged HTML/JS/CSS, workers.dev 404, preserved admin login, and logout/session revocation. Evidence is under `.local/deployment/onboarding/delivery-preflight.json` and `activation-smoke.json`. The challenge/role verification used three indexed record reads in total and no writes. The initial role read returned transient API 7403; account verification plus a single identical retry succeeded. No reset or cleanup was applied to the user's new club.

A deployment warning compared omitted nested sampling defaults with dashboard values. Independent settings readback at **16:53:12 UTC** confirmed effective log sampling remains **10%**, persisted invocation logs remain enabled, traces remain disabled, and the new version serves 100% of traffic. No corrective deployment or live log tail was needed. Cloudflare and Resend remain on Free; pay-as-you-go remains disabled. Full local application suites were not repeated for this configuration-only release; the preceding local results remain historical evidence. The 20-room/200-player regional load gate remains open.
