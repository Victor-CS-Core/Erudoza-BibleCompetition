...
### Remembered minimization

Eligible public visitors and Adult coaches can use the minus control beside the launcher (or blocked-provider fallback) to minimize support. The floating controls and their extra footer spacing disappear. The footer then offers **Support Erudoza ↗** and **Show floating support button**. Student mode and account pages keep their existing exclusions.

The versioned local-storage key `erudoza:coffee-minimized:v1` stores `1` for minimized and `0` for restored. It has no expiry and applies to this browser across account changes. Other tabs update through storage events; clearing site data resets the preference. If browser storage is unavailable, the preference still works across navigation for the current visit. Devices and separately isolated installed-app storage do not synchronize. This is a display preference, not a donation record.

The initial preference is read before exposing the provider launcher. Minimizing in another tab closes any open support popup and restores page interactivity, clears the scroll lock and moves focus to the footer. Explicit minimize/restore actions move keyboard focus to the footer support link or restored control. Shared native dialogs suppress floating support controls. The provider script may still load while minimized; no payment frame scraping, webhook, server preference, or account data was introduced. See the [minimization audit](../audits/2026-09-12-support-minimization.md).
